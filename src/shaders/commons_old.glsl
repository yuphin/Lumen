...
vec3 sample_light_Li(const vec4 rands_pos, const vec3 p, const int num_lights,
    ...
    case LIGHT_AREA: {
        ...
        TriangleRecord record =
            sample_area_light(rands_pos, light, material_idx, triangle_idx, light_record.bary);
...
vec3 sample_light_Li(const vec4 rands_pos, const vec3 p, const int num_lights,
                     ...
                     out float pdf_pos_a, out float cos_from_light,
                     out LightRecord light_record, out vec2 uv, out uint instance_idx, out uint triangle_idx) {
    ...
    case LIGHT_AREA: {
        ...
        uint material_idx;
        TriangleRecord record =
            sample_area_light(rands_pos, light, material_idx, triangle_idx, uv);
        Material light_mat = load_material(material_idx, uv_unused);
        wi = record.pos - p;
        float wi_len_sqr = dot(wi, wi);
        wi_len = sqrt(wi_len_sqr);
...
vec3 sample_light_Li(const vec4 rands_pos, const vec3 p, const int num_lights,
    ...
    uint unused_idx;
    uint unused_idx2;
    return sample_light_Li(rands_pos, p, num_lights, pdf_pos_w, wi, wi_len, pdf_pos_a, cos_from_light, light_record, unused_uv, unused_idx, unused_idx2);
}
...
vec3 sample_light_Le(const vec4 rands_pos, const vec2 rands_dir,
    ...
    case LIGHT_AREA: {
        ...
        TriangleRecord record = sample_area_light(
            rands_pos, light, material_idx, triangle_idx);
        Material light_mat = load_material(material_idx, uv_unused);
        pos = record.pos;
