...
vec3 sample_hemisphere(vec2 uv, vec3 n, inout float phi) {
    phi = TWO_PI * uv.x;
...
vec3 sample_hemisphere(vec2 uv, vec3 n) {
   float unused;
   return sample_hemisphere(uv, n, unused);
}
...
vec3 sample_bsdf(const vec3 n_s, const vec3 wo, const Material mat,
...
#if 1
        dir = sample_hemisphere(rands, n_s);
        f = diffuse_f(mat);
    ...
    case BSDF_TYPE_GLOSSY: {
        if (rands.x < .5) {
            ...
            dir = sample_hemisphere(rands_new, n_s);
        } else {
