...
void vcmmlt::init(Integrator* integrator) {
	...
	REGISTER_BUFFER_WITH_ADDRESS(SceneDesc, desc, vcm_vertices_addr, state.light_path_buffer, vk::render_graph());
	REGISTER_BUFFER_WITH_ADDRESS(SceneDesc, desc, path_cnt_addr, state.light_path_cnt_buffer, vk::render_graph());
	REGISTER_BUFFER_WITH_ADDRESS(SceneDesc, desc, color_storage_addr, state.tmp_col_buffer, vk::render_graph());
	REGISTER_BUFFER_WITH_ADDRESS(SceneDesc, desc, photon_addr, state.photon_buffer, vk::render_graph());
	REGISTER_BUFFER_WITH_ADDRESS(SceneDesc, desc, mlt_atomicsum_addr, state.mlt_atomicsum_buffer, vk::render_graph());
	REGISTER_BUFFER_WITH_ADDRESS(SceneDesc, desc, residual_addr, state.mlt_residual_buffer, vk::render_graph());
...
void vcmmlt::render(Integrator* integrator) {
	...
	rg->add_rt(pipeline_name,
		...
		.bind_tlas(*integrator->tlas);
	i32 counter = 0;
	prefix_scan(integrator, 0, config.num_bootstrap_samples, counter, rg);
	// Calculate CDF
