...
void VCMMLT::render() {
	...
	std::array<VkBufferMemoryBarrier, 2> barriers = {
		buffer_barrier(chain_stats_buffer.handle, VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT,
					   VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT),
		buffer_barrier(mlt_atomicsum_buffer.handle, VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT,
					   VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT),
	};
	...
	if (use_vm) {
		...
		auto barrier = buffer_barrier(photon_buffer.handle, VK_ACCESS_TRANSFER_WRITE_BIT,
									  VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT);
	...
	auto sum_up_chain_data = [&] {
		auto wg_x = 1024;
		auto wg_y = 1;
		const auto num_wg_x = (uint32_t)ceil(lumen_scene->config.num_mlt_threads / float(wg_x));
		const auto num_wg_y = 1;
		vkCmdBindDescriptorSets(cmd.handle, VK_PIPELINE_BIND_POINT_COMPUTE, sum0_pipeline->pipeline_layout, 0, 1,
								&desc_set, 0, nullptr);
		vkCmdBindDescriptorSets(cmd.handle, VK_PIPELINE_BIND_POINT_COMPUTE, sum1_pipeline->pipeline_layout, 0, 1,
								&desc_set, 0, nullptr);
		vkCmdBindDescriptorSets(cmd.handle, VK_PIPELINE_BIND_POINT_COMPUTE, sum_reduce0_pipeline->pipeline_layout, 0, 1,
								&desc_set, 0, nullptr);
		vkCmdBindDescriptorSets(cmd.handle, VK_PIPELINE_BIND_POINT_COMPUTE, sum_reduce1_pipeline->pipeline_layout, 0, 1,
								&desc_set, 0, nullptr);
		vkCmdPushConstants(cmd.handle, sum0_pipeline->pipeline_layout, VK_SHADER_STAGE_COMPUTE_BIT, 0,
						   sizeof(PushConstantRay), &pc_ray);
		vkCmdPushConstants(cmd.handle, sum1_pipeline->pipeline_layout, VK_SHADER_STAGE_COMPUTE_BIT, 0,
						   sizeof(PushConstantRay), &pc_ray);
		vkCmdPushConstants(cmd.handle, sum_reduce0_pipeline->pipeline_layout, VK_SHADER_STAGE_COMPUTE_BIT, 0,
						   sizeof(PushConstantRay), &pc_ray);
		vkCmdPushConstants(cmd.handle, sum_reduce1_pipeline->pipeline_layout, VK_SHADER_STAGE_COMPUTE_BIT, 0,
						   sizeof(PushConstantRay), &pc_ray);
		reduce(cmd.handle, mlt_residual_buffer, counter_buffer, *sum0_pipeline, *sum_reduce0_pipeline,
			   lumen_scene->config.num_mlt_threads);
		auto barrier = buffer_barrier(chain_stats_buffer.handle, VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT,
									  VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT);
		vkCmdPipelineBarrier(cmd.handle, VK_PIPELINE_STAGE_ALL_COMMANDS_BIT, VK_PIPELINE_STAGE_ALL_COMMANDS_BIT, 0, 0,
							 0, 1, &barrier, 0, 0);
		reduce(cmd.handle, mlt_residual_buffer, counter_buffer, *sum1_pipeline, *sum_reduce1_pipeline,
			   lumen_scene->config.num_mlt_threads);
		barrier = buffer_barrier(chain_stats_buffer.handle, VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT,
								 VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT);
		vkCmdPipelineBarrier(cmd.handle, VK_PIPELINE_STAGE_ALL_COMMANDS_BIT, VK_PIPELINE_STAGE_ALL_COMMANDS_BIT, 0, 0,
							 0, 1, &barrier, 0, 0);
	};
	// Shoot rays
	{
		...
		vkCmdBindPipeline(cmd.handle, VK_PIPELINE_BIND_POINT_RAY_TRACING_KHR, trace_pipeline->handle);
		vkCmdBindDescriptorSets(cmd.handle, VK_PIPELINE_BIND_POINT_RAY_TRACING_KHR, trace_pipeline->pipeline_layout, 0,
								1, &desc_set, 0, nullptr);
		vkCmdPushConstants(
			cmd.handle, trace_pipeline->pipeline_layout,
		...
		std::array<VkBufferMemoryBarrier, 3> barriers = {
			buffer_barrier(light_path_buffer.handle, VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT,
						   VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT),
			buffer_barrier(light_path_cnt_buffer.handle, VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT,
						   VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT),
			buffer_barrier(photon_buffer.handle, VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT,
						   VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT)};
		vkCmdPipelineBarrier(cmd.handle, VK_PIPELINE_STAGE_ALL_COMMANDS_BIT, VK_PIPELINE_STAGE_ALL_COMMANDS_BIT,
							 VK_DEPENDENCY_BY_REGION_BIT, 0, 0, barriers.size(), barriers.data(), 0, 0);
	}
	// Start bootstrap sampling
	{
		...
		vkCmdBindPipeline(cmd.handle, VK_PIPELINE_BIND_POINT_RAY_TRACING_KHR, seed_pipeline->handle);
		vkCmdBindDescriptorSets(cmd.handle, VK_PIPELINE_BIND_POINT_RAY_TRACING_KHR, seed_pipeline->pipeline_layout, 0,
								1, &desc_set, 0, nullptr);
		vkCmdPushConstants(
			cmd.handle, seed_pipeline->pipeline_layout,
			VK_SHADER_STAGE_RAYGEN_BIT_KHR | VK_SHADER_STAGE_CLOSEST_HIT_BIT_KHR | VK_SHADER_STAGE_MISS_BIT_KHR, 0,
			sizeof(PushConstantRay), &pc_ray);
		auto& regions = seed_pipeline->get_rt_regions();
		vkCmdTraceRaysKHR(cmd.handle, &regions[0], &regions[1], &regions[2], &regions[3],
						  lumen_scene->config.num_bootstrap_samples, 1, 1);
		auto barrier = buffer_barrier(bootstrap_buffer.handle, VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT,
									  VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT);
		vkCmdPipelineBarrier(cmd.handle, VK_PIPELINE_STAGE_ALL_COMMANDS_BIT, VK_PIPELINE_STAGE_ALL_COMMANDS_BIT, 0, 0,
							 0, 1, &barrier, 0, 0);
	}
	...
	// Calculate CDF
	{
		vkCmdBindDescriptorSets(cmd.handle, VK_PIPELINE_BIND_POINT_COMPUTE, calc_cdf_pipeline->pipeline_layout, 0, 1,
								&desc_set, 0, nullptr);
		vkCmdBindPipeline(cmd.handle, VK_PIPELINE_BIND_POINT_COMPUTE, calc_cdf_pipeline->handle);
		vkCmdPushConstants(cmd.handle, calc_cdf_pipeline->pipeline_layout, VK_SHADER_STAGE_COMPUTE_BIT, 0,
						   sizeof(PushConstantRay), &pc_ray);
		auto num_wgs = (lumen_scene->config.num_bootstrap_samples + 1023) / 1024;
		vkCmdDispatch(cmd.handle, num_wgs, 1, 1);
		std::array<VkBufferMemoryBarrier, 2> barriers = {
			buffer_barrier(cdf_sum_buffer.handle, VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT,
						   VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT),
			buffer_barrier(cdf_buffer.handle, VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT,
						   VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT)};
		vkCmdPipelineBarrier(cmd.handle, VK_PIPELINE_STAGE_ALL_COMMANDS_BIT, VK_PIPELINE_STAGE_ALL_COMMANDS_BIT, 0, 0,
							 0, (uint32_t)barriers.size(), barriers.data(), 0, 0);
	}

	// Select seeds
	{
		vkCmdBindDescriptorSets(cmd.handle, VK_PIPELINE_BIND_POINT_COMPUTE, select_seeds_pipeline->pipeline_layout, 0,
								1, &desc_set, 0, nullptr);
		vkCmdBindPipeline(cmd.handle, VK_PIPELINE_BIND_POINT_COMPUTE, select_seeds_pipeline->handle);
		vkCmdPushConstants(cmd.handle, select_seeds_pipeline->pipeline_layout, VK_SHADER_STAGE_COMPUTE_BIT, 0,
						   sizeof(PushConstantRay), &pc_ray);
		uint32_t num_wgs = (lumen_scene->config.num_mlt_threads + 1023) / 1024;
		vkCmdDispatch(cmd.handle, num_wgs, 1, 1);
		auto barrier = buffer_barrier(seeds_buffer.handle, VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT,
									  VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT);
		vkCmdPipelineBarrier(cmd.handle, VK_PIPELINE_STAGE_ALL_COMMANDS_BIT, VK_PIPELINE_STAGE_ALL_COMMANDS_BIT, 0, 0,
							 0, 1, &barrier, 0, 0);
	}
	// Fill in the samplers for mutations
	{
		...
		vkCmdBindPipeline(cmd.handle, VK_PIPELINE_BIND_POINT_RAY_TRACING_KHR, preprocess_pipeline->handle);
		vkCmdBindDescriptorSets(cmd.handle, VK_PIPELINE_BIND_POINT_RAY_TRACING_KHR,
								preprocess_pipeline->pipeline_layout, 0, 1, &desc_set, 0, nullptr);
		vkCmdPushConstants(
			cmd.handle, preprocess_pipeline->pipeline_layout,
			VK_SHADER_STAGE_RAYGEN_BIT_KHR | VK_SHADER_STAGE_CLOSEST_HIT_BIT_KHR | VK_SHADER_STAGE_MISS_BIT_KHR, 0,
			sizeof(PushConstantRay), &pc_ray);
		auto& preprocess_regions = preprocess_pipeline->get_rt_regions();
		vkCmdTraceRaysKHR(cmd.handle, &preprocess_regions[0], &preprocess_regions[1], &preprocess_regions[2],
						  &preprocess_regions[3], lumen_scene->config.num_mlt_threads, 1, 1);
		const std::array<VkBufferMemoryBarrier, 3> preprocess_barriers = {
			buffer_barrier(seeds_buffer.handle, VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT,
						   VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT),
			buffer_barrier(mlt_samplers_buffer.handle, VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT,
						   VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT),
			buffer_barrier(mlt_atomicsum_buffer.handle, VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT,
						   VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT)};
		vkCmdPipelineBarrier(cmd.handle, VK_PIPELINE_STAGE_ALL_COMMANDS_BIT, VK_PIPELINE_STAGE_ALL_COMMANDS_BIT, 0, 0,
							 0, (uint32_t)preprocess_barriers.size(), preprocess_barriers.data(), 0, 0);
		// Sum up chain stats
		sum_up_chain_data();
	}
	...
	{
		vkCmdBindDescriptorSets(cmd.handle, VK_PIPELINE_BIND_POINT_COMPUTE, normalize_pipeline->pipeline_layout, 0, 1,
								&desc_set, 0, nullptr);
		vkCmdBindPipeline(cmd.handle, VK_PIPELINE_BIND_POINT_COMPUTE, normalize_pipeline->handle);
		vkCmdPushConstants(cmd.handle, normalize_pipeline->pipeline_layout, VK_SHADER_STAGE_COMPUTE_BIT, 0,
						   sizeof(PushConstantRay), &pc_ray);
	...
	// Start mutations
	{
		auto mutate = [&](uint32_t i) {
			...
			// Mutate
			{
				...
				vkCmdBindPipeline(cmd.handle, VK_PIPELINE_BIND_POINT_RAY_TRACING_KHR, mutate_pipeline->handle);
				vkCmdBindDescriptorSets(cmd.handle, VK_PIPELINE_BIND_POINT_RAY_TRACING_KHR,
										mutate_pipeline->pipeline_layout, 0, 1, &desc_set, 0, nullptr);
				vkCmdPushConstants(
					cmd.handle, mutate_pipeline->pipeline_layout,
					VK_SHADER_STAGE_RAYGEN_BIT_KHR | VK_SHADER_STAGE_CLOSEST_HIT_BIT_KHR | VK_SHADER_STAGE_MISS_BIT_KHR,
					0, sizeof(PushConstantRay), &pc_ray);
				auto& regions = mutate_pipeline->get_rt_regions();
				vkCmdTraceRaysKHR(cmd.handle, &regions[0], &regions[1], &regions[2], &regions[3],
								  lumen_scene->config.num_mlt_threads, 1, 1);
				std::array<VkBufferMemoryBarrier, 6> mutation_barriers = {
					buffer_barrier(splat_buffer.handle, VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT,
								   VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT),
					buffer_barrier(past_splat_buffer.handle, VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT,
								   VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT),
					buffer_barrier(mlt_samplers_buffer.handle, VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT,
								   VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT),
					buffer_barrier(seeds_buffer.handle, VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT,
								   VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT),
					buffer_barrier(mlt_col_buffer.handle, VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT,
								   VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT),
					buffer_barrier(mlt_atomicsum_buffer.handle, VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT,
								   VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT),
				};
				vkCmdPipelineBarrier(cmd.handle, VK_PIPELINE_STAGE_ALL_COMMANDS_BIT, VK_PIPELINE_STAGE_ALL_COMMANDS_BIT,
									 0, 0, 0, (uint32_t)mutation_barriers.size(), mutation_barriers.data(), 0, 0);
				// Sum up chain stats
				sum_up_chain_data();
			}
			// Normalization
			{
				vkCmdBindDescriptorSets(cmd.handle, VK_PIPELINE_BIND_POINT_COMPUTE, normalize_pipeline->pipeline_layout,
	...
	cmd.begin();
	{
		vkCmdBindDescriptorSets(cmd.handle, VK_PIPELINE_BIND_POINT_COMPUTE, composite_pipeline->pipeline_layout, 0, 1,
								&desc_set, 0, nullptr);
		vkCmdBindPipeline(cmd.handle, VK_PIPELINE_BIND_POINT_COMPUTE, composite_pipeline->handle);
		vkCmdPushConstants(cmd.handle, composite_pipeline->pipeline_layout, VK_SHADER_STAGE_COMPUTE_BIT, 0,
						   sizeof(PushConstantRay), &pc_ray);
...
void VCMMLT::create_rt_pipelines() {
	enum StageIndices { Raygen = 0, CMiss = 1, AMiss = 2, ClosestHit = 3, AnyHit = 4, ShaderGroupCount = 5 };
	RTPipelineSettings settings;
	std::vector<Shader> shaders{{"src/shaders/integrators/vcmmlt/vcmmlt_eye.rgen"},
...
void VCMMLT::create_compute_pipelines() {
	...
	std::vector<Shader> shaders = {
		{"src/shaders/integrators/pssmlt/calc_cdf.comp"},	 {"src/shaders/integrators/vcmmlt/select_seeds.comp"},
		{"src/shaders/integrators/vcmmlt/composite.comp"},	 {"src/shaders/integrators/pssmlt/prefix_scan.comp"},
		{"src/shaders/integrators/pssmlt/uniform_add.comp"}, {"src/shaders/integrators/vcmmlt/normalize.comp"},
		{"src/shaders/integrators/vcmmlt/sum.comp"},		 {"src/shaders/integrators/vcmmlt/reduce_sum.comp"},
	};
	for (auto& shader : shaders) {
		shader.compile();
	}
...
void VCMMLT::prefix_scan(int level, int num_elems, CommandBuffer& cmd) {
	...
	auto scan = [&](int num_wgs, int idx) {
		vkCmdBindDescriptorSets(cmd.handle, VK_PIPELINE_BIND_POINT_COMPUTE, prefix_scan_pipeline->pipeline_layout, 0, 1,
								&desc_set, 0, nullptr);
		vkCmdBindPipeline(cmd.handle, VK_PIPELINE_BIND_POINT_COMPUTE, prefix_scan_pipeline->handle);
		vkCmdPushConstants(cmd.handle, prefix_scan_pipeline->pipeline_layout, VK_SHADER_STAGE_COMPUTE_BIT, 0,
						   sizeof(PushConstantCompute), &pc_compute);
		vkCmdDispatch(cmd.handle, num_wgs, 1, 1);
		if (scan_sums) {
			auto barrier =
				buffer_barrier(block_sums[idx].handle, VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT,
							   VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT);
			vkCmdPipelineBarrier(cmd.handle, VK_PIPELINE_STAGE_ALL_COMMANDS_BIT, VK_PIPELINE_STAGE_ALL_COMMANDS_BIT, 0,
								 0, 0, 1, &barrier, 0, 0);
		} else {
			auto barrier = buffer_barrier(cdf_buffer.handle, VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT,
										  VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT);
			vkCmdPipelineBarrier(cmd.handle, VK_PIPELINE_STAGE_ALL_COMMANDS_BIT, VK_PIPELINE_STAGE_ALL_COMMANDS_BIT, 0,
								 0, 0, 1, &barrier, 0, 0);
		}
	...
	auto uniform_add = [&](int num_wgs, int output_idx) {
		vkCmdBindDescriptorSets(cmd.handle, VK_PIPELINE_BIND_POINT_COMPUTE, uniform_add_pipeline->pipeline_layout, 0, 1,
								&desc_set, 0, nullptr);
		vkCmdBindPipeline(cmd.handle, VK_PIPELINE_BIND_POINT_COMPUTE, uniform_add_pipeline->handle);
		vkCmdPushConstants(cmd.handle, uniform_add_pipeline->pipeline_layout, VK_SHADER_STAGE_COMPUTE_BIT, 0,
						   sizeof(PushConstantCompute), &pc_compute);
		vkCmdDispatch(cmd.handle, num_wgs, 1, 1);
		if (scan_sums) {
			auto barrier =
				buffer_barrier(block_sums[output_idx].handle, VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT,
							   VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT);
			vkCmdPipelineBarrier(cmd.handle, VK_PIPELINE_STAGE_ALL_COMMANDS_BIT, VK_PIPELINE_STAGE_ALL_COMMANDS_BIT, 0,
								 0, 0, 1, &barrier, 0, 0);
		} else {
...
void VCMMLT::destroy() {
	...
	std::vector<Pipeline*> pipeline_list = {
		eye_pipeline.get(),			light_pipeline.get(),		 seed1_pipeline.get(),	   seed2_pipeline.get(),
		preprocess1_pipeline.get(), preprocess2_pipeline.get(),	 mutate1_pipeline.get(),   mutate2_pipeline.get(),
		calc_cdf_pipeline.get(),	select_seeds_pipeline.get(), composite_pipeline.get(), prefix_scan_pipeline.get(),
		uniform_add_pipeline.get(), normalize_pipeline.get(),	 sum0_pipeline.get(),	   sum1_pipeline.get(),
		sum_reduce0_pipeline.get(), sum_reduce1_pipeline.get()};
