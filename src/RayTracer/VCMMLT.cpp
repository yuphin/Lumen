...
void VCMMLT::init() {
	Integrator::init();
	mutation_count = int(instance->width * instance->height *
						 lumen_scene->config.mutations_per_pixel /
						 float(lumen_scene->config.num_mlt_threads));
	light_path_rand_count = std::max(7 + 2 * lumen_scene->config.path_length,
									 3 + 6 * lumen_scene->config.path_length);
	...
	light_primary_samples_buffer.create(
		...
		VK_BUFFER_USAGE_STORAGE_BUFFER_BIT |
			...
			VK_BUFFER_USAGE_TRANSFER_DST_BIT,
		VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, VK_SHARING_MODE_EXCLUSIVE,
		lumen_scene->config.num_mlt_threads * light_path_rand_count *
			sizeof(PrimarySample) * 2);
	...
	chain_stats_buffer.create(&instance->vkb.ctx,
							  VK_BUFFER_USAGE_STORAGE_BUFFER_BIT |
								  ...
								  VK_BUFFER_USAGE_TRANSFER_DST_BIT,
							  VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT,
							  VK_SHARING_MODE_EXCLUSIVE, 2 * sizeof(ChainData));
	...
	splat_buffer.create(&instance->vkb.ctx,
						...
						lumen_scene->config.num_mlt_threads *
							(lumen_scene->config.path_length *
							 (lumen_scene->config.path_length + 1)) *
							sizeof(Splat) * 2);
	...
	desc.light_primary_samples_addr =
		light_primary_samples_buffer.get_device_address();
...
void VCMMLT::render() {
	...
	pc_ray.radius =
		lumen_scene->m_dimensions.radius * vcm_radius_factor / 100.f;
	pc_ray.radius /=
		(float)pow((double)pc_ray.frame_num + 1, 0.5 * (1 - 2.0 / 3));
	...
	vkCmdFillBuffer(cmd.handle, chain_stats_buffer.handle, 0,
					chain_stats_buffer.size, 0);
	vkCmdFillBuffer(cmd.handle, mlt_atomicsum_buffer.handle, 0,
					mlt_atomicsum_buffer.size, 0);
	...
	vkCmdPipelineBarrier(cmd.handle, VK_PIPELINE_STAGE_ALL_COMMANDS_BIT,
						 ...
						 barriers.data(), 0, 0);
	if (use_vm) {
		vkCmdFillBuffer(cmd.handle, photon_buffer.handle, 0, photon_buffer.size,
						0);
		auto barrier = buffer_barrier(
			photon_buffer.handle, VK_ACCESS_TRANSFER_WRITE_BIT,
			VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT);
	...
	auto sum_up_chain_data = [&] {
		...
		auto wg_y = 1;
		const auto num_wg_x =
			(uint32_t)ceil(lumen_scene->config.num_mlt_threads / float(wg_x));
		const auto num_wg_y = 1;
		vkCmdBindDescriptorSets(cmd.handle, VK_PIPELINE_BIND_POINT_COMPUTE,
								sum0_pipeline->pipeline_layout, 0, 1, &desc_set,
								0, nullptr);
		vkCmdBindDescriptorSets(cmd.handle, VK_PIPELINE_BIND_POINT_COMPUTE,
								sum1_pipeline->pipeline_layout, 0, 1, &desc_set,
								0, nullptr);
		vkCmdBindDescriptorSets(cmd.handle, VK_PIPELINE_BIND_POINT_COMPUTE,
								sum_reduce0_pipeline->pipeline_layout, 0, 1,
								&desc_set, 0, nullptr);
		vkCmdBindDescriptorSets(cmd.handle, VK_PIPELINE_BIND_POINT_COMPUTE,
								sum_reduce1_pipeline->pipeline_layout, 0, 1,
								&desc_set, 0, nullptr);
		...
		vkCmdPushConstants(cmd.handle, sum_reduce1_pipeline->pipeline_layout,
						   VK_SHADER_STAGE_COMPUTE_BIT, 0,
						   sizeof(PushConstantRay), &pc_ray);
		reduce(cmd.handle, mlt_residual_buffer, counter_buffer, *sum0_pipeline,
			   *sum_reduce0_pipeline, lumen_scene->config.num_mlt_threads);
		auto barrier = buffer_barrier(
			chain_stats_buffer.handle,
			VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT,
			VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT);
		vkCmdPipelineBarrier(cmd.handle, VK_PIPELINE_STAGE_ALL_COMMANDS_BIT,
							 VK_PIPELINE_STAGE_ALL_COMMANDS_BIT, 0, 0, 0, 1,
							 &barrier, 0, 0);
		reduce(cmd.handle, mlt_residual_buffer, counter_buffer, *sum1_pipeline,
			   *sum_reduce1_pipeline, lumen_scene->config.num_mlt_threads);
	...
	{
		auto trace_pipeline =
			!light_first ? eye_pipeline.get() : light_pipeline.get();
		...
		vkCmdBindDescriptorSets(
			...
			trace_pipeline->pipeline_layout, 0, 1, &desc_set, 0, nullptr);
		...
		std::array<VkBufferMemoryBarrier, 3> barriers = {
			buffer_barrier(
				light_path_buffer.handle,
				VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT,
				VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT),
			buffer_barrier(
				light_path_cnt_buffer.handle,
				VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT,
				VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT),
			buffer_barrier(
				photon_buffer.handle,
				VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT,
				VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT)};
		vkCmdPipelineBarrier(cmd.handle, VK_PIPELINE_STAGE_ALL_COMMANDS_BIT,
							 VK_PIPELINE_STAGE_ALL_COMMANDS_BIT,
	...
	{
		auto seed_pipeline =
			!light_first ? seed1_pipeline.get() : seed2_pipeline.get();
		vkCmdBindPipeline(cmd.handle, VK_PIPELINE_BIND_POINT_RAY_TRACING_KHR,
						  seed_pipeline->handle);
		vkCmdBindDescriptorSets(
			cmd.handle, VK_PIPELINE_BIND_POINT_RAY_TRACING_KHR,
			seed_pipeline->pipeline_layout, 0, 1, &desc_set, 0, nullptr);
	...
	{
		vkCmdBindDescriptorSets(cmd.handle, VK_PIPELINE_BIND_POINT_COMPUTE,
								calc_cdf_pipeline->pipeline_layout, 0, 1,
								&desc_set, 0, nullptr);
		...
		vkCmdPushConstants(cmd.handle, calc_cdf_pipeline->pipeline_layout,
						   ...
						   sizeof(PushConstantRay), &pc_ray);
		auto num_wgs =
			(lumen_scene->config.num_bootstrap_samples + 1023) / 1024;
		vkCmdDispatch(cmd.handle, num_wgs, 1, 1);
		std::array<VkBufferMemoryBarrier, 2> barriers = {
			buffer_barrier(
				cdf_sum_buffer.handle,
				VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT,
				VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT),
			buffer_barrier(
				cdf_buffer.handle,
		...
		vkCmdPipelineBarrier(cmd.handle, VK_PIPELINE_STAGE_ALL_COMMANDS_BIT,
							 ...
							 (uint32_t)barriers.size(), barriers.data(), 0, 0);
	}

	// Select seeds
	{
		vkCmdBindDescriptorSets(cmd.handle, VK_PIPELINE_BIND_POINT_COMPUTE,
								select_seeds_pipeline->pipeline_layout, 0, 1,
								&desc_set, 0, nullptr);
		vkCmdBindPipeline(cmd.handle, VK_PIPELINE_BIND_POINT_COMPUTE,
	...
	{
		// Fill
		auto preprocess_pipeline = !light_first ? preprocess1_pipeline.get()
		...
		vkCmdTraceRaysKHR(cmd.handle, &preprocess_regions[0],
						  &preprocess_regions[1], &preprocess_regions[2],
						  &preprocess_regions[3],
		...
		const std::array<VkBufferMemoryBarrier, 3> preprocess_barriers = {
			buffer_barrier(
				...
				VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT),
			buffer_barrier(
				mlt_samplers_buffer.handle,
				VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT,
				VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT),
			buffer_barrier(
				mlt_atomicsum_buffer.handle,
				VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT,
				VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT)};
		vkCmdPipelineBarrier(cmd.handle, VK_PIPELINE_STAGE_ALL_COMMANDS_BIT,
							 VK_PIPELINE_STAGE_ALL_COMMANDS_BIT, 0, 0, 0,
	...
	{
		vkCmdBindDescriptorSets(cmd.handle, VK_PIPELINE_BIND_POINT_COMPUTE,
								normalize_pipeline->pipeline_layout, 0, 1,
								&desc_set, 0, nullptr);
		vkCmdBindPipeline(cmd.handle, VK_PIPELINE_BIND_POINT_COMPUTE,
		...
		auto mutate = [&](uint32_t i) {
			...
			// Mutate
			{
				...
				vkCmdPipelineBarrier(
					cmd.handle, VK_PIPELINE_STAGE_ALL_COMMANDS_BIT,
					VK_PIPELINE_STAGE_ALL_COMMANDS_BIT,
					VK_DEPENDENCY_BY_REGION_BIT, 0, 0, 1, &barrier, 0, 0);
				auto mutate_pipeline = !light_first ? mutate1_pipeline.get()
				...
				vkCmdBindDescriptorSets(cmd.handle,
										VK_PIPELINE_BIND_POINT_RAY_TRACING_KHR,
										mutate_pipeline->pipeline_layout, 0, 1,
										&desc_set, 0, nullptr);
				...
				vkCmdTraceRaysKHR(cmd.handle, &regions[0], &regions[1],
								  &regions[2], &regions[3],
								  lumen_scene->config.num_mlt_threads, 1, 1);
				...
				vkCmdPipelineBarrier(cmd.handle,
									 ...
									 0, (uint32_t)mutation_barriers.size(),
			...
			// Normalization
			{
				vkCmdBindDescriptorSets(cmd.handle,
										...
										normalize_pipeline->pipeline_layout, 0,
										1, &desc_set, 0, nullptr);
				vkCmdBindPipeline(cmd.handle, VK_PIPELINE_BIND_POINT_COMPUTE,
								  normalize_pipeline->handle);
				vkCmdPushConstants(cmd.handle,
								   normalize_pipeline->pipeline_layout,
								   VK_SHADER_STAGE_COMPUTE_BIT, 0,
								   sizeof(PushConstantRay), &pc_ray);
				vkCmdDispatch(cmd.handle, 1, 1, 1);
			}
	...
	{
		vkCmdBindDescriptorSets(cmd.handle, VK_PIPELINE_BIND_POINT_COMPUTE,
								composite_pipeline->pipeline_layout, 0, 1,
								&desc_set, 0, nullptr);
		vkCmdBindPipeline(cmd.handle, VK_PIPELINE_BIND_POINT_COMPUTE,
...
bool VCMMLT::gui() {
	bool result = false;
	result |= ImGui::Checkbox("Enable Light-first ordering(default = eye)",
							  &light_first);
	if (light_first) {
...
void VCMMLT::create_rt_pipelines() {
	...
	settings.shaders = {shaders[0], shaders[4], shaders[5], shaders[6],
						shaders[7]};
	...
	vkDestroyShaderModule(instance->vkb.ctx.device, stages[Raygen].module,
						  nullptr);
	stages[Raygen].module =
		shaders[3].create_vk_shader_module(instance->vkb.ctx.device);
...
void VCMMLT::prefix_scan(int level, int num_elems, CommandBuffer& cmd) {
	...
	auto scan = [&](int num_wgs, int idx) {
		vkCmdBindDescriptorSets(cmd.handle, VK_PIPELINE_BIND_POINT_COMPUTE,
								prefix_scan_pipeline->pipeline_layout, 0, 1,
								&desc_set, 0, nullptr);
		vkCmdBindPipeline(cmd.handle, VK_PIPELINE_BIND_POINT_COMPUTE,
	...
	auto uniform_add = [&](int num_wgs, int output_idx) {
		vkCmdBindDescriptorSets(cmd.handle, VK_PIPELINE_BIND_POINT_COMPUTE,
								uniform_add_pipeline->pipeline_layout, 0, 1,
								&desc_set, 0, nullptr);
		vkCmdBindPipeline(cmd.handle, VK_PIPELINE_BIND_POINT_COMPUTE,
	...
	if (num_wgs > 1) {
		...
		if (scan_sums) {
			pc_compute.block_sum_addr =
				block_sums[level - 1].get_device_address();
		}
...
void VCMMLT::destroy() {
	...
	std::vector<Buffer*> buffer_list = {
		&bootstrap_buffer,	   &cdf_buffer,
		&cdf_sum_buffer,	   &seeds_buffer,
		&mlt_samplers_buffer,  &light_primary_samples_buffer,
		&mlt_col_buffer,	   &chain_stats_buffer,
		&splat_buffer,		   &past_splat_buffer,
		&light_path_buffer,	   &light_path_cnt_buffer,
		&tmp_col_buffer,	   &photon_buffer,
		&mlt_atomicsum_buffer, &mlt_residual_buffer,
		&counter_buffer};
	...
	std::vector<Pipeline*> pipeline_list = {
		eye_pipeline.get(),			light_pipeline.get(),
		seed1_pipeline.get(),		seed2_pipeline.get(),
		preprocess1_pipeline.get(), preprocess2_pipeline.get(),
		mutate1_pipeline.get(),		mutate2_pipeline.get(),
		calc_cdf_pipeline.get(),	select_seeds_pipeline.get(),
		composite_pipeline.get(),	prefix_scan_pipeline.get(),
		uniform_add_pipeline.get(), normalize_pipeline.get(),
		sum0_pipeline.get(),		sum1_pipeline.get(),
