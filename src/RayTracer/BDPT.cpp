...
void BDPT::init() {
	...
	light_path_buffer.create(
		...
		instance->width * instance->height *
			(lumen_scene->config.path_length + 1) * sizeof(PathVertex));
	...
	REGISTER_BUFFER_WITH_ADDRESS(SceneDesc, desc, light_path_addr,
								 &light_path_buffer, instance->vkb.rg);
	REGISTER_BUFFER_WITH_ADDRESS(SceneDesc, desc, camera_path_addr,
								 &camera_path_buffer, instance->vkb.rg);
	REGISTER_BUFFER_WITH_ADDRESS(SceneDesc, desc, color_storage_addr,
								 &color_storage_buffer, instance->vkb.rg);
}
...
void BDPT::destroy() {
	...
	Integrator::destroy();
	std::vector<Buffer*> buffer_list = {&light_path_buffer, &camera_path_buffer,
