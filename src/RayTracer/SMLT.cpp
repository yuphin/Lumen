...
void SMLT::render() {
	...
	// Start bootstrap sampling
	{
		// Fill light buffer
		{
			vkCmdBindPipeline(cmd.handle, VK_PIPELINE_BIND_POINT_RAY_TRACING_KHR, seed_light_pipeline->handle);
			vkCmdBindDescriptorSets(cmd.handle, VK_PIPELINE_BIND_POINT_RAY_TRACING_KHR,
									seed_light_pipeline->pipeline_layout, 0, 1, &desc_set, 0, nullptr);
			vkCmdPushConstants(
				cmd.handle, seed_light_pipeline->pipeline_layout,
			...
			vkCmdTraceRaysKHR(cmd.handle, &regions[0], &regions[1], &regions[2], &regions[3], num_bootstrap_samples, 1,
							  1);
			std::array<VkBufferMemoryBarrier, 3> barriers = {
				buffer_barrier(tmp_lum_buffer.handle, VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT,
							   VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT),
				buffer_barrier(tmp_seeds_buffer.handle, VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT,
							   VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT),
				buffer_barrier(connected_lights_buffer.handle, VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT,
							   VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT),
			};
			vkCmdPipelineBarrier(cmd.handle, VK_PIPELINE_STAGE_ALL_COMMANDS_BIT, VK_PIPELINE_STAGE_ALL_COMMANDS_BIT, 0,
								 0, 0, (uint32_t)barriers.size(), barriers.data(), 0, 0);
		}
		// Trace from eye
		{
			vkCmdBindPipeline(cmd.handle, VK_PIPELINE_BIND_POINT_RAY_TRACING_KHR, seed_eye_pipeline->handle);
			vkCmdBindDescriptorSets(cmd.handle, VK_PIPELINE_BIND_POINT_RAY_TRACING_KHR,
									seed_eye_pipeline->pipeline_layout, 0, 1, &desc_set, 0, nullptr);
			vkCmdPushConstants(
				cmd.handle, seed_eye_pipeline->pipeline_layout,
				VK_SHADER_STAGE_RAYGEN_BIT_KHR | VK_SHADER_STAGE_CLOSEST_HIT_BIT_KHR | VK_SHADER_STAGE_MISS_BIT_KHR, 0,
				sizeof(PushConstantRay), &pc_ray);
			...
			vkCmdTraceRaysKHR(cmd.handle, &regions[0], &regions[1], &regions[2], &regions[3], num_bootstrap_samples, 1,
							  1);
			auto barrier =
				buffer_barrier(bootstrap_buffer.handle, VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT,
							   VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT);
			vkCmdPipelineBarrier(cmd.handle, VK_PIPELINE_STAGE_ALL_COMMANDS_BIT, VK_PIPELINE_STAGE_ALL_COMMANDS_BIT, 0,
								 0, 0, 1, &barrier, 0, 0);
		}
	...
	// Fill in the samplers for mutations
	{
		// Light
		{
			...
			auto barrier = buffer_barrier(mlt_samplers_buffer.handle, VK_ACCESS_TRANSFER_WRITE_BIT,
										  VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT);
			...
			vkCmdBindPipeline(cmd.handle, VK_PIPELINE_BIND_POINT_RAY_TRACING_KHR, preprocess_light_pipeline->handle);
			vkCmdBindDescriptorSets(cmd.handle, VK_PIPELINE_BIND_POINT_RAY_TRACING_KHR,
									preprocess_light_pipeline->pipeline_layout, 0, 1, &desc_set, 0, nullptr);
			vkCmdPushConstants(
				cmd.handle, preprocess_light_pipeline->pipeline_layout,
				VK_SHADER_STAGE_RAYGEN_BIT_KHR | VK_SHADER_STAGE_CLOSEST_HIT_BIT_KHR | VK_SHADER_STAGE_MISS_BIT_KHR, 0,
				sizeof(PushConstantRay), &pc_ray);
			...
			vkCmdTraceRaysKHR(cmd.handle, &preprocess_regions[0], &preprocess_regions[1], &preprocess_regions[2],
							  &preprocess_regions[3], num_mlt_threads, 1, 1);
			std::array<VkBufferMemoryBarrier, 6> barriers = {
				...
				buffer_barrier(light_path_cnt_buffer.handle, VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT,
							   VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT),
				buffer_barrier(light_splats_buffer.handle, VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT,
							   VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT),
				buffer_barrier(light_splat_cnts_buffer.handle, VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT,
							   VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT),
				buffer_barrier(connected_lights_buffer.handle, VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT,
							   VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT),
			};
		...
		// Eye
		{
			vkCmdBindPipeline(cmd.handle, VK_PIPELINE_BIND_POINT_RAY_TRACING_KHR, preprocess_eye_pipeline->handle);
			vkCmdBindDescriptorSets(cmd.handle, VK_PIPELINE_BIND_POINT_RAY_TRACING_KHR,
									preprocess_eye_pipeline->pipeline_layout, 0, 1, &desc_set, 0, nullptr);
			vkCmdPushConstants(
				cmd.handle, preprocess_eye_pipeline->pipeline_layout,
				VK_SHADER_STAGE_RAYGEN_BIT_KHR | VK_SHADER_STAGE_CLOSEST_HIT_BIT_KHR | VK_SHADER_STAGE_MISS_BIT_KHR, 0,
				sizeof(PushConstantRay), &pc_ray);
	...
	// Start mutations
	{
		auto mutate = [&](uint32_t i) {
			...
			// Light
			{
				vkCmdBindPipeline(cmd.handle, VK_PIPELINE_BIND_POINT_RAY_TRACING_KHR, mutate_light_pipeline->handle);
				vkCmdBindDescriptorSets(cmd.handle, VK_PIPELINE_BIND_POINT_RAY_TRACING_KHR,
										mutate_light_pipeline->pipeline_layout, 0, 1, &desc_set, 0, nullptr);
				vkCmdPushConstants(
					cmd.handle, mutate_light_pipeline->pipeline_layout,
					VK_SHADER_STAGE_RAYGEN_BIT_KHR | VK_SHADER_STAGE_CLOSEST_HIT_BIT_KHR | VK_SHADER_STAGE_MISS_BIT_KHR,
					0, sizeof(PushConstantRay), &pc_ray);
				auto& regions = mutate_light_pipeline->get_rt_regions();
				vkCmdTraceRaysKHR(cmd.handle, &regions[0], &regions[1], &regions[2], &regions[3], num_mlt_threads, 1,
								  1);
				std::array<VkBufferMemoryBarrier, 6> mutation_barriers = {
					...
					buffer_barrier(tmp_seeds_buffer.handle, VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT,
								   VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT),
					buffer_barrier(prob_carryover_buffer.handle, VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT,
								   VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT),
			...
			// Eye
			{
				vkCmdBindPipeline(cmd.handle, VK_PIPELINE_BIND_POINT_RAY_TRACING_KHR, mutate_eye_pipeline->handle);
				vkCmdBindDescriptorSets(cmd.handle, VK_PIPELINE_BIND_POINT_RAY_TRACING_KHR,
										mutate_eye_pipeline->pipeline_layout, 0, 1, &desc_set, 0, nullptr);
				vkCmdPushConstants(
					cmd.handle, mutate_eye_pipeline->pipeline_layout,
					VK_SHADER_STAGE_RAYGEN_BIT_KHR | VK_SHADER_STAGE_CLOSEST_HIT_BIT_KHR | VK_SHADER_STAGE_MISS_BIT_KHR,
					0, sizeof(PushConstantRay), &pc_ray);
...
void SMLT::create_compute_pipelines() {
	...
	std::vector<Shader> shaders = {
		{"src/shaders/integrators/pssmlt/calc_cdf.comp"},	 {"src/shaders/integrators/pssmlt/select_seeds.comp"},
		{"src/shaders/integrators/pssmlt/composite.comp"},	 {"src/shaders/integrators/pssmlt/prefix_scan.comp"},
...
void SMLT::destroy() {
	...
	std::vector<Pipeline*> pipeline_list = {
		seed_light_pipeline.get(),	   seed_eye_pipeline.get(),		preprocess_light_pipeline.get(),
		preprocess_eye_pipeline.get(), mutate_light_pipeline.get(), mutate_eye_pipeline.get(),
		calc_cdf_pipeline.get(),	   select_seeds_pipeline.get(), composite_pipeline.get(),
		prefix_scan_pipeline.get(),	   uniform_add_pipeline.get()};
