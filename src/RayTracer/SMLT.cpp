...
void SMLT::init() {
	...
	mutation_count = int(instance->width * instance->height *
						 mutations_per_pixel / float(num_mlt_threads));
	...
	desc.cam_primary_samples_addr =
		cam_primary_samples_buffer.get_device_address();
...
void SMLT::render() {
	...
	// Start bootstrap sampling
	{
		// Fill light buffer
		{
			...
			vkCmdBindDescriptorSets(cmd.handle,
									VK_PIPELINE_BIND_POINT_RAY_TRACING_KHR,
									seed_light_pipeline->pipeline_layout, 0, 1,
									&desc_set, 0, nullptr);
		...
		// Trace from eye
		{
			...
			vkCmdBindDescriptorSets(cmd.handle,
									VK_PIPELINE_BIND_POINT_RAY_TRACING_KHR,
									seed_eye_pipeline->pipeline_layout, 0, 1,
									&desc_set, 0, nullptr);
	...
	// Fill in the samplers for mutations
	{
		// Light
		{
			vkCmdFillBuffer(cmd.handle, mlt_samplers_buffer.handle, 0,
							mlt_samplers_buffer.size, 0);
			auto barrier = buffer_barrier(
				mlt_samplers_buffer.handle, VK_ACCESS_TRANSFER_WRITE_BIT,
				VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT);
			...
			vkCmdBindDescriptorSets(cmd.handle,
									VK_PIPELINE_BIND_POINT_RAY_TRACING_KHR,
									preprocess_light_pipeline->pipeline_layout,
			...
			vkCmdPushConstants(cmd.handle,
							   ...
							   VK_SHADER_STAGE_RAYGEN_BIT_KHR |
								   ...
								   VK_SHADER_STAGE_MISS_BIT_KHR,
							   0, sizeof(PushConstantRay), &pc_ray);
			auto& preprocess_regions =
				preprocess_light_pipeline->get_rt_regions();
			...
			std::array<VkBufferMemoryBarrier, 6> barriers = {
				...
				buffer_barrier(
					...
					VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT),
				buffer_barrier(
					light_splats_buffer.handle,
					VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT,
					VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT),
				buffer_barrier(
					light_splat_cnts_buffer.handle,
					VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT,
					VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT),
				buffer_barrier(
					connected_lights_buffer.handle,
		...
		// Eye
		{
			...
			vkCmdBindDescriptorSets(cmd.handle,
									VK_PIPELINE_BIND_POINT_RAY_TRACING_KHR,
									preprocess_eye_pipeline->pipeline_layout, 0,
									1, &desc_set, 0, nullptr);
			vkCmdPushConstants(cmd.handle,
							   preprocess_eye_pipeline->pipeline_layout,
							   VK_SHADER_STAGE_RAYGEN_BIT_KHR |
								   VK_SHADER_STAGE_CLOSEST_HIT_BIT_KHR |
								   VK_SHADER_STAGE_MISS_BIT_KHR,
							   0, sizeof(PushConstantRay), &pc_ray);
			auto& preprocess_regions =
				preprocess_eye_pipeline->get_rt_regions();
			vkCmdTraceRaysKHR(cmd.handle, &preprocess_regions[0],
							  &preprocess_regions[1], &preprocess_regions[2],
							  &preprocess_regions[3], num_mlt_threads, 1, 1);
		}
	...
	// Start mutations
	{
		auto mutate = [&](uint32_t i) {
			...
			// Light
			{
				...
				vkCmdBindDescriptorSets(cmd.handle,
										VK_PIPELINE_BIND_POINT_RAY_TRACING_KHR,
										mutate_light_pipeline->pipeline_layout,
										0, 1, &desc_set, 0, nullptr);
				vkCmdPushConstants(cmd.handle,
								   mutate_light_pipeline->pipeline_layout,
								   VK_SHADER_STAGE_RAYGEN_BIT_KHR |
				...
				vkCmdTraceRaysKHR(cmd.handle, &regions[0], &regions[1],
								  &regions[2], &regions[3], num_mlt_threads, 1,
								  1);
			...
			// Eye
			{
				...
				vkCmdBindDescriptorSets(cmd.handle,
										VK_PIPELINE_BIND_POINT_RAY_TRACING_KHR,
										mutate_eye_pipeline->pipeline_layout, 0,
										1, &desc_set, 0, nullptr);
				vkCmdPushConstants(cmd.handle,
								   mutate_eye_pipeline->pipeline_layout,
								   VK_SHADER_STAGE_RAYGEN_BIT_KHR |
									   VK_SHADER_STAGE_CLOSEST_HIT_BIT_KHR |
									   VK_SHADER_STAGE_MISS_BIT_KHR,
								   0, sizeof(PushConstantRay), &pc_ray);
...
void SMLT::create_rt_pipelines() {
	...
	std::vector<Shader> shaders{
		{"src/shaders/integrators/smlt/smlt_seed_light.rgen"},
	...
	vkDestroyShaderModule(instance->vkb.ctx.device, stages[Raygen].module,
						  nullptr);
	stages[Raygen].module =
		shaders[4].create_vk_shader_module(instance->vkb.ctx.device);
	...
	vkDestroyShaderModule(instance->vkb.ctx.device, stages[Raygen].module,
						  nullptr);
	stages[Raygen].module =
		shaders[5].create_vk_shader_module(instance->vkb.ctx.device);
...
void SMLT::destroy() {
	...
	std::vector<Pipeline*> pipeline_list = {
		seed_light_pipeline.get(),		 seed_eye_pipeline.get(),
		preprocess_light_pipeline.get(), preprocess_eye_pipeline.get(),
		mutate_light_pipeline.get(),	 mutate_eye_pipeline.get(),
