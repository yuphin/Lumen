...
void PSSMLT::init() {
	...
	cam_primary_samples_buffer.create(
		...
		VK_BUFFER_USAGE_STORAGE_BUFFER_BIT |
			...
			VK_BUFFER_USAGE_TRANSFER_DST_BIT,
		VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, VK_SHARING_MODE_EXCLUSIVE,
		lumen_scene->config.num_mlt_threads * cam_path_rand_count *
			sizeof(PrimarySample));
	...
	connection_primary_samples_buffer.create(
		...
		VK_BUFFER_USAGE_STORAGE_BUFFER_BIT |
			...
			VK_BUFFER_USAGE_TRANSFER_DST_BIT,
		VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, VK_SHARING_MODE_EXCLUSIVE,
		lumen_scene->config.num_mlt_threads * connect_path_rand_count *
			sizeof(PrimarySample));
	...
	past_splat_buffer.create(&instance->vkb.ctx,
							 ...
							 lumen_scene->config.num_mlt_threads *
								 (lumen_scene->config.path_length *
								  (lumen_scene->config.path_length + 1)) *
								 sizeof(Splat));
	auto path_size = std::max(lumen_scene->config.num_mlt_threads,
							  lumen_scene->config.num_bootstrap_samples);
	light_path_buffer.create(&instance->vkb.ctx,
							 ...
							 VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT,
							 VK_SHARING_MODE_EXCLUSIVE,
							 path_size * (lumen_scene->config.path_length + 1) *
								 sizeof(MLTPathVertex));
	...
	desc.connection_primary_samples_addr =
		connection_primary_samples_buffer.get_device_address();
...
void PSSMLT::render() {
	...
	auto zero_buffers = [&]() {
		vkCmdFillBuffer(cmd.handle, light_path_buffer.handle, 0,
						light_path_buffer.size, 0);
		vkCmdFillBuffer(cmd.handle, camera_path_buffer.handle, 0,
						camera_path_buffer.size, 0);
	...
	// Start mutations
	{
		auto mutate = [&](uint32_t i) {
			...
			vkCmdTraceRaysKHR(cmd.handle, &regions[0], &regions[1], &regions[2],
							  &regions[3], lumen_scene->config.num_mlt_threads,
							  1, 1);
...
void PSSMLT::create_rt_pipelines() {
	...
	std::vector<Shader> shaders{
		{"src/shaders/integrators/pssmlt/pssmlt_seed.rgen"},
	...
	settings.stages = stages;
	settings.shaders = {shaders[1], shaders[3], shaders[4], shaders[5],
						shaders[6]};
	...
	settings.stages = stages;
	settings.shaders = {shaders[2], shaders[3], shaders[4], shaders[5],
						shaders[6]};
...
void PSSMLT::destroy() {
	...
	std::vector<Pipeline*> pipeline_list = {
		seed_pipeline.get(),		 preprocess_pipeline.get(),
		mutate_pipeline.get(),		 calc_cdf_pipeline.get(),
		select_seeds_pipeline.get(), composite_pipeline.get(),
