...
void PSSMLT::init() {
	...
	cdf_cpu.create(&instance->vkb.ctx, VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT,
				   VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
				   VK_SHARING_MODE_EXCLUSIVE, lumen_scene->config.num_bootstrap_samples * sizeof(float));
	...
	camera_path_buffer.create(&instance->vkb.ctx,
							  VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT |
								  VK_BUFFER_USAGE_TRANSFER_DST_BIT,
							  VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, VK_SHARING_MODE_EXCLUSIVE,
							  path_size * (lumen_scene->config.path_length + 1) * sizeof(MLTPathVertex));
...
void PSSMLT::render() {
	...
	auto zero_buffers = [&]() {
		...
		std::array<VkBufferMemoryBarrier, 2> barriers = {
			...
			buffer_barrier(camera_path_buffer.handle, VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT,
						   VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT),
		};
	...
	// Start mutations
	{
		auto mutate = [&](uint32_t i) {
			...
			vkCmdPipelineBarrier(cmd.handle, VK_PIPELINE_STAGE_ALL_COMMANDS_BIT, VK_PIPELINE_STAGE_ALL_COMMANDS_BIT, 0,
								 0, 0, (uint32_t)mutation_barriers.size(), mutation_barriers.data(), 0, 0);
		};
...
void PSSMLT::destroy() {
	...
	std::vector<Pipeline*> pipeline_list = {
		seed_pipeline.get(),		 preprocess_pipeline.get(), mutate_pipeline.get(),		calc_cdf_pipeline.get(),
		select_seeds_pipeline.get(), composite_pipeline.get(),	prefix_scan_pipeline.get(), uniform_add_pipeline.get()};
