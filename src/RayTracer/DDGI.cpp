...
void DDGI::init() {
	...
	// DDGI Resources
	{
		...
		const int irradiance_width =
			(IRRADIANCE_SIDE_LENGTH + 2) * probe_counts.x * probe_counts.y;
		const int irradiance_height =
			(IRRADIANCE_SIDE_LENGTH + 2) * probe_counts.z;
		const int depth_width =
			(DEPTH_SIDE_LENGTH + 2) * probe_counts.x * probe_counts.y;
		...
		for (int i = 0; i < 2; i++) {
			settings.base_extent = {(uint32_t)irradiance_width,
									(uint32_t)irradiance_height, 1};
			...
			irr_texes[i].create_empty_texture(&instance->vkb.ctx, settings,
											  ...
											  bilinear_sampler);
			settings.base_extent = {(uint32_t)depth_width,
		...
		{
			TextureSettings settings;
			settings.base_extent = {(uint32_t)rays_per_probe,
	...
	ddgi_ubo_buffer.create(&instance->vkb.ctx,
	...
	// Samplers
	{
		...
		vk::check(vkCreateSampler(instance->vkb.ctx.device, &sampler_ci,
								  nullptr, &bilinear_sampler),
...
void DDGI::render() {
	...
	// Generate random orientation for probes
	{
		...
		pc_ray.probe_rotation = glm::mat4_cast(glm::angleAxis(
			2.0f * glm::pi<float>() * rands.x,
			glm::normalize(glm::vec3(rands.y, rands.z, rands.w))));
	}
	// Clear color image
	...
	vkCmdClearColorImage(cmd.handle, output_tex.img,
						 VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, &clear_color_val,
						 1, &isr);
	...
	// Trace Primary rays and fill G buffer
	{
		...
		vkCmdBindDescriptorSets(
			...
			vis_pipeline->pipeline_layout, 0, 1, &scene_desc_set, 0, nullptr);
	...
	// Trace rays from probes
	{
		...
		vkCmdPipelineBarrier(
			cmd.handle, VK_PIPELINE_STAGE_RAY_TRACING_SHADER_BIT_KHR,
			VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, VK_DEPENDENCY_BY_REGION_BIT,
			0, 0, 0, 0, image_barriers.size(), image_barriers.data());

		vkCmdBindPipeline(cmd.handle, VK_PIPELINE_BIND_POINT_RAY_TRACING_KHR,
						  probe_trace_pipeline->handle);
		std::array<VkDescriptorSet, 4> descriptor_sets = {
			scene_desc_set, ddgi_uniform.desc_set, rt.write_desc_set,
			rt.read_desc_set};
		vkCmdBindDescriptorSets(
			cmd.handle, VK_PIPELINE_BIND_POINT_RAY_TRACING_KHR,
		...
		vkCmdTraceRaysKHR(cmd.handle, &regions[0], &regions[1], &regions[2],
						  &regions[3], rays_per_probe, grid_size, 1);
		image_barriers = {
	...
	// Classify
	{
		vkCmdBindPipeline(cmd.handle, VK_PIPELINE_BIND_POINT_COMPUTE,
						  classify_pipeline->handle);
		std::array<VkDescriptorSet, 3> descriptor_sets = {
			scene_desc_set, ddgi_uniform.desc_set, rt.read_desc_set};
		...
		auto wg_x =
			(probe_counts.x * probe_counts.y * probe_counts.z + 31) / 32;

		vkCmdBindDescriptorSets(cmd.handle, VK_PIPELINE_BIND_POINT_COMPUTE,
								classify_pipeline->pipeline_layout, 0,
	...
	// Update probes & borders
	{
		auto update_probe = [&](bool is_irr) {
			...
			std::array<VkDescriptorSet, 5> descriptor_sets = {
				scene_desc_set, ddgi.write_desc_sets[!ping_pong],
		...
		// Probes
		{
			std::array<VkImageMemoryBarrier, 2> image_barriers = {
				image_barrier(
					irr_texes[!ping_pong].img, VK_ACCESS_SHADER_READ_BIT,
					VK_ACCESS_SHADER_WRITE_BIT, VK_IMAGE_LAYOUT_UNDEFINED,
					VK_IMAGE_LAYOUT_GENERAL, VK_IMAGE_ASPECT_COLOR_BIT),
				image_barrier(
					depth_texes[!ping_pong].img, VK_ACCESS_SHADER_READ_BIT,
					VK_ACCESS_SHADER_WRITE_BIT, VK_IMAGE_LAYOUT_UNDEFINED,
					VK_IMAGE_LAYOUT_GENERAL, VK_IMAGE_ASPECT_COLOR_BIT)};
			...
			image_barriers = {
				image_barrier(
					irr_texes[!ping_pong].img, VK_ACCESS_SHADER_WRITE_BIT,
					VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT,
					VK_IMAGE_LAYOUT_GENERAL, VK_IMAGE_LAYOUT_GENERAL,
					VK_IMAGE_ASPECT_COLOR_BIT),
				image_barrier(
					depth_texes[!ping_pong].img, VK_ACCESS_SHADER_WRITE_BIT,
					VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT,
					VK_IMAGE_LAYOUT_GENERAL, VK_IMAGE_LAYOUT_GENERAL,
					VK_IMAGE_ASPECT_COLOR_BIT)};
			vkCmdPipelineBarrier(cmd.handle,
		...
		{
			vkCmdBindPipeline(cmd.handle, VK_PIPELINE_BIND_POINT_COMPUTE,
							  border_update_pipeline->handle);
			std::array<VkDescriptorSet, 2> descriptor_sets = {
				ddgi.write_desc_sets[!ping_pong], ddgi_uniform.desc_set};
			vkCmdPushConstants(cmd.handle,
							   border_update_pipeline->pipeline_layout,
							   VK_SHADER_STAGE_COMPUTE_BIT, 0,
							   sizeof(PushConstantRay), &pc_ray);
			// 13 WGs process 4 probes (wg = 32 threads)
			auto wg_x =
				(probe_counts.x * probe_counts.y * probe_counts.z + 3) * 13 / 4;

			vkCmdBindDescriptorSets(cmd.handle, VK_PIPELINE_BIND_POINT_COMPUTE,
									border_update_pipeline->pipeline_layout, 0,
									descriptor_sets.size(),
									descriptor_sets.data(), 0, nullptr);
			vkCmdDispatch(cmd.handle, wg_x, 1, 1);
			std::array<VkImageMemoryBarrier, 2> image_barriers = {
				...
				image_barrier(
					...
					VK_IMAGE_ASPECT_COLOR_BIT)};
			vkCmdPipelineBarrier(
				cmd.handle, VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT,
				VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT |
					VK_PIPELINE_STAGE_RAY_TRACING_SHADER_BIT_KHR,
				VK_DEPENDENCY_BY_REGION_BIT, 0, 0, 0, 0, image_barriers.size(),
				image_barriers.data());
		}
	}
	...
	// Sample probes & output into texture
	{
		...
		vkCmdPipelineBarrier(cmd.handle, VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT,
							 ...
							 image_barriers.size(), image_barriers.data());
		vkCmdBindPipeline(cmd.handle, VK_PIPELINE_BIND_POINT_COMPUTE,
						  sample_pipeline->handle);
		vkCmdPushConstants(cmd.handle, sample_pipeline->pipeline_layout,
						   VK_SHADER_STAGE_COMPUTE_BIT, 0,
						   sizeof(PushConstantRay), &pc_ray);
		std::array<VkDescriptorSet, 4> descriptor_sets = {
			scene_desc_set, output.write_desc_set,
			ddgi.read_desc_sets[ping_pong], ddgi_uniform.desc_set};
		vkCmdBindDescriptorSets(cmd.handle, VK_PIPELINE_BIND_POINT_COMPUTE,
								sample_pipeline->pipeline_layout, 0,
								descriptor_sets.size(), descriptor_sets.data(),
								0, nullptr);
	...
	if (total_frame_idx < 5) {
		{
			...
			vkCmdBindDescriptorSets(cmd.handle, VK_PIPELINE_BIND_POINT_COMPUTE,
									relocate_pipeline->pipeline_layout, 0,
									descriptor_sets.size(),
									descriptor_sets.data(), 0, nullptr);
			vkCmdDispatch(cmd.handle, wg_x, 1, 1);
			...
			vkCmdPipelineBarrier(cmd.handle, VK_PIPELINE_STAGE_ALL_COMMANDS_BIT,
								 ...
								 barriers.size(), barriers.data(), 0, 0);
		}
	}
	...
	// Output
	{
		...
		vkCmdPushConstants(cmd.handle, out_pipeline->pipeline_layout,
						   VK_SHADER_STAGE_COMPUTE_BIT, 0,
						   sizeof(PushConstantRay), &pc_ray);
		std::array<VkDescriptorSet, 2> descriptor_sets = {scene_desc_set,
														  output.read_desc_set};
		vkCmdBindDescriptorSets(cmd.handle, VK_PIPELINE_BIND_POINT_COMPUTE,
								out_pipeline->pipeline_layout, 0,
								descriptor_sets.size(), descriptor_sets.data(),
								0, nullptr);
...
void DDGI::create_descriptors() {
	...
	auto descriptor_pool_ci =
		vk::descriptor_pool_CI(pool_sizes.size(), pool_sizes.data(), 10);
	...
	std::vector<VkWriteDescriptorSet> writes{
		...
		vk::write_descriptor_set(
			scene_desc_set, VK_DESCRIPTOR_TYPE_STORAGE_IMAGE, IMAGE_BINDING,
			&output_tex.descriptor_image_info),
		vk::write_descriptor_set(
			scene_desc_set, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, INSTANCE_BINDING,
			&prim_lookup_buffer.descriptor),
		vk::write_descriptor_set(
			scene_desc_set, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER,
			UNIFORM_BUFFER_BINDING, &scene_ubo_buffer.descriptor),
		vk::write_descriptor_set(
			scene_desc_set, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER,
	...
	writes.push_back(vk::write_descriptor_set(
		scene_desc_set, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER,
	...
	auto set_allocate_info = vk::descriptor_set_allocate_info(
		desc_pool, &ddgi_uniform.desc_layout, 1);
	vk::check(
		vkAllocateDescriptorSets(instance->vkb.ctx.device, &set_allocate_info,
...
// DDGI IRR & Depth
{
	...
	set_layout_ci = vk::descriptor_set_layout_CI(set_layout_bindings.data(),
												 set_layout_bindings.size());
	vk::check(
		vkCreateDescriptorSetLayout(instance->vkb.ctx.device, &set_layout_ci,
									nullptr, &ddgi.read_desc_layout),
		"Failed to create descriptor set layout");
	VkDescriptorSetLayout layouts[2] = {ddgi.write_desc_layout,
										ddgi.read_desc_layout};
	for (int i = 0; i < 2; i++) {
		VkDescriptorSet* sets[2] = {&ddgi.write_desc_sets[i],
									&ddgi.read_desc_sets[i]};
		for (int j = 0; j < 2; j++) {
			auto set_allocate_info =
				vk::descriptor_set_allocate_info(desc_pool, &layouts[j], 1);
			vk::check(vkAllocateDescriptorSets(instance->vkb.ctx.device,
											   &set_allocate_info, sets[j]),
					  "Failed to allocate descriptor sets");
		}
	}
}
// RT
{
	...
	vk::check(
		...
		"Failed to create descriptor set layout");
	VkDescriptorSetLayout layouts[2] = {rt.write_desc_layout,
										rt.read_desc_layout};
...
// DDGI Output
{
	...
	vk::check(
		...
		"Failed to create descriptor set layout");
	VkDescriptorSetLayout layouts[2] = {output.write_desc_layout,
...
// Update
{
	// UBO
	{
		VkWriteDescriptorSet wds = vk::write_descriptor_set(
			ddgi_uniform.desc_set, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, 0,
	...
	{
		std::vector<VkWriteDescriptorSet> writes = {
			vk::write_descriptor_set(rt.write_desc_set,
	...
	// RT Read
	{
		std::vector<VkDescriptorImageInfo> read_infos = {
			rt.radiance_tex.get_descriptor_info(
				nearest_sampler, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL),
			rt.dir_depth_tex.get_descriptor_info(
				nearest_sampler, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL),
		};
		std::vector<VkWriteDescriptorSet> writes = {
			vk::write_descriptor_set(rt.read_desc_set,
									 VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER,
									 0, &read_infos[0]),
			vk::write_descriptor_set(rt.read_desc_set,
									 VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER,
	...
	// Probe Writes
	{
		...
		for (int i = 0; i < 2; i++) {
			writes.push_back(vk::write_descriptor_set(
				ddgi.write_desc_sets[i], VK_DESCRIPTOR_TYPE_STORAGE_IMAGE, 0,
				&infos[2 * i]));
			writes.push_back(vk::write_descriptor_set(
				ddgi.write_desc_sets[i], VK_DESCRIPTOR_TYPE_STORAGE_IMAGE, 1,
				&infos[2 * i + 1]));
		}
		vkUpdateDescriptorSets(instance->vkb.ctx.device,
							   static_cast<uint32_t>(writes.size()),
	...
	{
		std::vector<VkDescriptorImageInfo> infos = {
			irr_texes[0].get_descriptor_info(
				bilinear_sampler, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL),
			depth_texes[0].get_descriptor_info(
				bilinear_sampler, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL),
			irr_texes[1].get_descriptor_info(
				bilinear_sampler, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL),
			depth_texes[1].get_descriptor_info(
				bilinear_sampler, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL)};
		std::vector<VkWriteDescriptorSet> writes;
		for (int i = 0; i < 2; i++) {
			writes.push_back(vk::write_descriptor_set(
				ddgi.read_desc_sets[i],
				VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, 0, &infos[2 * i]));
			writes.push_back(vk::write_descriptor_set(
				ddgi.read_desc_sets[i],
...
VkWriteDescriptorSet write_data_w = vk::write_descriptor_set(
	output.write_desc_set, VK_DESCRIPTOR_TYPE_STORAGE_IMAGE, 0, &wdi);
auto rdi = output.tex.get_descriptor_info(
	bilinear_sampler, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
VkWriteDescriptorSet write_data_r = vk::write_descriptor_set(
	output.read_desc_set, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, 0, &rdi);
...
void DDGI::create_rt_pipelines() {
	...
	settings.shaders = {shaders[0], shaders[2], shaders[3], shaders[4],
						shaders[5]};
	...
	settings.desc_layouts = {scene_desc_set_layout, ddgi_uniform.desc_layout,
							 rt.write_desc_layout, rt.read_desc_layout};
