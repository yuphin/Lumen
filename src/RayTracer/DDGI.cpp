...
void DDGI::init() {
	...
	// DDGI Resources
	{
		...
		for (int i = 0; i < 2; i++) {
			...
			irr_texes[i].create_empty_texture(&instance->vkb.ctx, settings, VK_IMAGE_LAYOUT_GENERAL, bilinear_sampler);
			settings.base_extent = {(uint32_t)depth_width, (uint32_t)depth_height, 1};
			settings.format = VK_FORMAT_R16G16_SFLOAT;
			depth_texes[i].create_empty_texture(&instance->vkb.ctx, settings, VK_IMAGE_LAYOUT_GENERAL,
												bilinear_sampler);
		}
		...
		// RT
		{
			...
			settings.format = VK_FORMAT_R16G16B16A16_SFLOAT;
			settings.usage_flags = VK_IMAGE_USAGE_STORAGE_BIT | VK_IMAGE_USAGE_SAMPLED_BIT;
			rt.radiance_tex.create_empty_texture(&instance->vkb.ctx, settings, VK_IMAGE_LAYOUT_GENERAL,
												 nearest_sampler);
			rt.dir_depth_tex.create_empty_texture(&instance->vkb.ctx, settings, VK_IMAGE_LAYOUT_GENERAL,
												  nearest_sampler);
		}
		// DDGI Output
		...
		output.tex.create_empty_texture(&instance->vkb.ctx, settings, VK_IMAGE_LAYOUT_GENERAL, bilinear_sampler);
	}
	...
	direct_lighting_buffer.create(&instance->vkb.ctx,
								  VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT,
								  VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, VK_SHARING_MODE_EXCLUSIVE,
								  instance->width * instance->height * sizeof(glm::vec3));

	ddgi_ubo_buffer.create(&instance->vkb.ctx, VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
						   VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
						   VK_SHARING_MODE_EXCLUSIVE, sizeof(DDGIUniforms));

	probe_offsets_buffer.create(&instance->vkb.ctx,
								VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT |
									VK_BUFFER_USAGE_TRANSFER_DST_BIT,
								VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, VK_SHARING_MODE_EXCLUSIVE,
...
void DDGI::render() {
	...
	isr.layerCount = 1;
	transition_image_layout(cmd.handle, output_tex.img, VK_IMAGE_LAYOUT_GENERAL, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
							VK_IMAGE_ASPECT_COLOR_BIT);
	vkCmdClearColorImage(cmd.handle, output_tex.img, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, &clear_color_val, 1, &isr);
	transition_image_layout(cmd.handle, output_tex.img, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_IMAGE_LAYOUT_GENERAL,
							VK_IMAGE_ASPECT_COLOR_BIT);
	if (first_frame) {
		transition_image_layout(cmd.handle, irr_texes[ping_pong].img, VK_IMAGE_LAYOUT_UNDEFINED,
								VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
		transition_image_layout(cmd.handle, depth_texes[ping_pong].img, VK_IMAGE_LAYOUT_UNDEFINED,
								VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
	}
	// Trace Primary rays and fill G buffer
	{
		vkCmdBindPipeline(cmd.handle, VK_PIPELINE_BIND_POINT_RAY_TRACING_KHR, vis_pipeline->handle);
		vkCmdBindDescriptorSets(cmd.handle, VK_PIPELINE_BIND_POINT_RAY_TRACING_KHR, vis_pipeline->pipeline_layout, 0, 1,
								&scene_desc_set, 0, nullptr);
		vkCmdPushConstants(
			cmd.handle, vis_pipeline->pipeline_layout,
		...
		vkCmdTraceRaysKHR(cmd.handle, &regions[0], &regions[1], &regions[2], &regions[3], instance->width,
						  instance->height, 1);
		auto barrier = buffer_barrier(g_buffer.handle, VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT,
	...
	// Trace rays from probes
	{
		std::array<VkImageMemoryBarrier, 2> image_barriers = {
			image_barrier(rt.radiance_tex.img, VK_ACCESS_SHADER_READ_BIT, VK_ACCESS_SHADER_WRITE_BIT,
						  VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_GENERAL, VK_IMAGE_ASPECT_COLOR_BIT),
			image_barrier(rt.dir_depth_tex.img, VK_ACCESS_SHADER_READ_BIT, VK_ACCESS_SHADER_WRITE_BIT,
						  VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_GENERAL, VK_IMAGE_ASPECT_COLOR_BIT)};
		vkCmdPipelineBarrier(cmd.handle, VK_PIPELINE_STAGE_RAY_TRACING_SHADER_BIT_KHR,
							 VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, VK_DEPENDENCY_BY_REGION_BIT, 0, 0, 0, 0,
							 image_barriers.size(), image_barriers.data());

		vkCmdBindPipeline(cmd.handle, VK_PIPELINE_BIND_POINT_RAY_TRACING_KHR, probe_trace_pipeline->handle);
		std::array<VkDescriptorSet, 4> descriptor_sets = {scene_desc_set, ddgi_uniform.desc_set, rt.write_desc_set,
														  rt.read_desc_set};
		vkCmdBindDescriptorSets(cmd.handle, VK_PIPELINE_BIND_POINT_RAY_TRACING_KHR,
								probe_trace_pipeline->pipeline_layout, 0, descriptor_sets.size(),
								descriptor_sets.data(), 0, nullptr);
		vkCmdPushConstants(
			cmd.handle, probe_trace_pipeline->pipeline_layout,
			VK_SHADER_STAGE_RAYGEN_BIT_KHR | VK_SHADER_STAGE_CLOSEST_HIT_BIT_KHR | VK_SHADER_STAGE_MISS_BIT_KHR, 0,
			sizeof(PushConstantRay), &pc_ray);
		...
		image_barriers = {
			image_barrier(rt.radiance_tex.img, VK_ACCESS_SHADER_WRITE_BIT, VK_ACCESS_SHADER_READ_BIT,
						  VK_IMAGE_LAYOUT_GENERAL, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, VK_IMAGE_ASPECT_COLOR_BIT),
			image_barrier(rt.dir_depth_tex.img, VK_ACCESS_SHADER_WRITE_BIT, VK_ACCESS_SHADER_READ_BIT,
	...
	// Classify
	{
		...
		std::array<VkDescriptorSet, 3> descriptor_sets = {scene_desc_set, ddgi_uniform.desc_set, rt.read_desc_set};
		vkCmdPushConstants(cmd.handle, classify_pipeline->pipeline_layout, VK_SHADER_STAGE_COMPUTE_BIT, 0,
						   sizeof(PushConstantRay), &pc_ray);
		// 13 WGs process 4 probes (wg = 32 threads)
		auto wg_x = (probe_counts.x * probe_counts.y * probe_counts.z + 31) / 32;

		vkCmdBindDescriptorSets(cmd.handle, VK_PIPELINE_BIND_POINT_COMPUTE, classify_pipeline->pipeline_layout, 0,
		...
		std::array<VkBufferMemoryBarrier, 1> barriers = {
			buffer_barrier(probe_offsets_buffer.handle, VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT,
						   VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT),
	...
	// Update probes & borders
	{
		auto update_probe = [&](bool is_irr) {
			...
			std::array<VkDescriptorSet, 5> descriptor_sets = {scene_desc_set, ddgi.write_desc_sets[!ping_pong],
															  ddgi.read_desc_sets[ping_pong], ddgi_uniform.desc_set,
															  rt.read_desc_set};
			vkCmdPushConstants(cmd.handle, pipeline->pipeline_layout, VK_SHADER_STAGE_COMPUTE_BIT, 0,
							   sizeof(PushConstantRay), &pc_ray);
			auto wg_x = probe_counts.x * probe_counts.y;
			auto wg_y = probe_counts.z;
			vkCmdBindDescriptorSets(cmd.handle, VK_PIPELINE_BIND_POINT_COMPUTE, pipeline->pipeline_layout, 0,
									descriptor_sets.size(), descriptor_sets.data(), 0, nullptr);
		...
		{
			std::array<VkImageMemoryBarrier, 2> image_barriers = {
				image_barrier(irr_texes[!ping_pong].img, VK_ACCESS_SHADER_READ_BIT, VK_ACCESS_SHADER_WRITE_BIT,
							  VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_GENERAL, VK_IMAGE_ASPECT_COLOR_BIT),
				image_barrier(depth_texes[!ping_pong].img, VK_ACCESS_SHADER_READ_BIT, VK_ACCESS_SHADER_WRITE_BIT,
							  VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_GENERAL, VK_IMAGE_ASPECT_COLOR_BIT)};
			vkCmdPipelineBarrier(cmd.handle, VK_PIPELINE_STAGE_RAY_TRACING_SHADER_BIT_KHR,
								 VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, VK_DEPENDENCY_BY_REGION_BIT, 0, 0, 0, 0,
								 image_barriers.size(), image_barriers.data());
			...
			image_barriers = {
				...
				image_barrier(depth_texes[!ping_pong].img, VK_ACCESS_SHADER_WRITE_BIT,
							  VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT, VK_IMAGE_LAYOUT_GENERAL,
							  VK_IMAGE_LAYOUT_GENERAL, VK_IMAGE_ASPECT_COLOR_BIT)};
			vkCmdPipelineBarrier(cmd.handle, VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT,
		...
		// Borders
		{
			...
			std::array<VkDescriptorSet, 2> descriptor_sets = {ddgi.write_desc_sets[!ping_pong], ddgi_uniform.desc_set};
			vkCmdPushConstants(cmd.handle, border_update_pipeline->pipeline_layout, VK_SHADER_STAGE_COMPUTE_BIT, 0,
							   sizeof(PushConstantRay), &pc_ray);
			// 13 WGs process 4 probes (wg = 32 threads)
			auto wg_x = (probe_counts.x * probe_counts.y * probe_counts.z + 3) * 13 / 4;

			vkCmdBindDescriptorSets(cmd.handle, VK_PIPELINE_BIND_POINT_COMPUTE, border_update_pipeline->pipeline_layout,
									0, descriptor_sets.size(), descriptor_sets.data(), 0, nullptr);
			vkCmdDispatch(cmd.handle, wg_x, 1, 1);
			std::array<VkImageMemoryBarrier, 2> image_barriers = {
				image_barrier(irr_texes[!ping_pong].img, VK_ACCESS_SHADER_WRITE_BIT, VK_ACCESS_SHADER_READ_BIT,
							  VK_IMAGE_LAYOUT_GENERAL, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL,
							  VK_IMAGE_ASPECT_COLOR_BIT),
				image_barrier(depth_texes[!ping_pong].img, VK_ACCESS_SHADER_WRITE_BIT, VK_ACCESS_SHADER_READ_BIT,
							  VK_IMAGE_LAYOUT_GENERAL, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL,
							  VK_IMAGE_ASPECT_COLOR_BIT)};
	...
	// Sample probes & output into texture
	{
		std::array<VkImageMemoryBarrier, 1> image_barriers = {
			image_barrier(output.tex.img, VK_ACCESS_SHADER_READ_BIT, VK_ACCESS_SHADER_WRITE_BIT,
						  VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_GENERAL, VK_IMAGE_ASPECT_COLOR_BIT)};
		vkCmdPipelineBarrier(cmd.handle, VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT, VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT,
							 VK_DEPENDENCY_BY_REGION_BIT, 0, 0, 0, 0, image_barriers.size(), image_barriers.data());
		vkCmdBindPipeline(cmd.handle, VK_PIPELINE_BIND_POINT_COMPUTE, sample_pipeline->handle);
		vkCmdPushConstants(cmd.handle, sample_pipeline->pipeline_layout, VK_SHADER_STAGE_COMPUTE_BIT, 0,
						   sizeof(PushConstantRay), &pc_ray);
		std::array<VkDescriptorSet, 4> descriptor_sets = {scene_desc_set, output.write_desc_set,
														  ddgi.read_desc_sets[ping_pong], ddgi_uniform.desc_set};
		vkCmdBindDescriptorSets(cmd.handle, VK_PIPELINE_BIND_POINT_COMPUTE, sample_pipeline->pipeline_layout, 0,
								descriptor_sets.size(), descriptor_sets.data(), 0, nullptr);

		auto wg_x = (instance->width + 31) / 32;
		auto wg_y = (instance->height + 31) / 32;
		vkCmdDispatch(cmd.handle, wg_x, wg_y, 1);
		image_barriers = {image_barrier(output.tex.img, VK_ACCESS_SHADER_WRITE_BIT, VK_ACCESS_SHADER_READ_BIT,
										VK_IMAGE_LAYOUT_GENERAL, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL,
										VK_IMAGE_ASPECT_COLOR_BIT)};
		vkCmdPipelineBarrier(cmd.handle, VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT,
							 VK_DEPENDENCY_BY_REGION_BIT, 0, 0, 0, 0, image_barriers.size(), image_barriers.data());
	}
	...
	if (total_frame_idx < 5) {
		{
			...
			std::array<VkDescriptorSet, 3> descriptor_sets = {scene_desc_set, ddgi_uniform.desc_set, rt.read_desc_set};
			vkCmdPushConstants(cmd.handle, relocate_pipeline->pipeline_layout, VK_SHADER_STAGE_COMPUTE_BIT, 0,
							   sizeof(PushConstantRay), &pc_ray);
			// 13 WGs process 4 probes (wg = 32 threads)
			auto wg_x = (probe_counts.x * probe_counts.y * probe_counts.z + 31) / 32;

			vkCmdBindDescriptorSets(cmd.handle, VK_PIPELINE_BIND_POINT_COMPUTE, relocate_pipeline->pipeline_layout, 0,
									descriptor_sets.size(), descriptor_sets.data(), 0, nullptr);
			vkCmdDispatch(cmd.handle, wg_x, 1, 1);
	...
	// Output
	{
		...
		vkCmdPushConstants(cmd.handle, out_pipeline->pipeline_layout, VK_SHADER_STAGE_COMPUTE_BIT, 0,
						   sizeof(PushConstantRay), &pc_ray);
		std::array<VkDescriptorSet, 2> descriptor_sets = {scene_desc_set, output.read_desc_set};
		vkCmdBindDescriptorSets(cmd.handle, VK_PIPELINE_BIND_POINT_COMPUTE, out_pipeline->pipeline_layout, 0,
								descriptor_sets.size(), descriptor_sets.data(), 0, nullptr);
...
void DDGI::create_descriptors() {
	...
	std::vector<VkDescriptorPoolSize> pool_sizes = {
		...
		vk::descriptor_pool_size(VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, num_textures + 5),
	...
	auto descriptor_pool_ci = vk::descriptor_pool_CI(pool_sizes.size(), pool_sizes.data(), 10);
	...
	std::vector<VkDescriptorSetLayoutBinding> set_layout_bindings = {
		...
		vk::descriptor_set_layout_binding(
			...
			INSTANCE_BINDING),
		vk::descriptor_set_layout_binding(VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, VK_SHADER_STAGE_ALL,
	...
	vk::check(vkCreateDescriptorSetLayout(instance->vkb.ctx.device, &set_layout_ci, nullptr, &scene_desc_set_layout),
	...
	std::vector<VkWriteDescriptorSet> writes{
		vk::write_descriptor_set(scene_desc_set, VK_DESCRIPTOR_TYPE_ACCELERATION_STRUCTURE_KHR, TLAS_BINDING,
								 &desc_as_info),
		vk::write_descriptor_set(scene_desc_set, VK_DESCRIPTOR_TYPE_STORAGE_IMAGE, IMAGE_BINDING,
								 &output_tex.descriptor_image_info),
		vk::write_descriptor_set(scene_desc_set, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, INSTANCE_BINDING,
								 &prim_lookup_buffer.descriptor),
		vk::write_descriptor_set(scene_desc_set, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, UNIFORM_BUFFER_BINDING,
								 &scene_ubo_buffer.descriptor),
		vk::write_descriptor_set(scene_desc_set, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, SCENE_DESC_BINDING,
								 &scene_desc_buffer.descriptor)};
	std::vector<VkDescriptorImageInfo> image_infos;
	for (auto& tex : textures) {
		image_infos.push_back(tex.descriptor_image_info);
	}
	writes.push_back(vk::write_descriptor_set(scene_desc_set, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER,
											  TEXTURES_BINDING, image_infos.data(), (uint32_t)image_infos.size()));
	if (lights.size()) {
		writes.push_back(vk::write_descriptor_set(scene_desc_set, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, LIGHTS_BINDING,
	...
	{// UBO
	 {std::vector<VkDescriptorSetLayoutBinding> set_layout_bindings = {
		  vk::descriptor_set_layout_binding(VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, VK_SHADER_STAGE_ALL, 0)};
	auto set_layout_ci = vk::descriptor_set_layout_CI(set_layout_bindings.data(), set_layout_bindings.size());
	vk::check(vkCreateDescriptorSetLayout(instance->vkb.ctx.device, &set_layout_ci, nullptr, &ddgi_uniform.desc_layout),
			  "Failed to create descriptor set layout");

	auto set_allocate_info = vk::descriptor_set_allocate_info(desc_pool, &ddgi_uniform.desc_layout, 1);
	vk::check(vkAllocateDescriptorSets(instance->vkb.ctx.device, &set_allocate_info, &ddgi_uniform.desc_set),
...
// DDGI IRR & Depth
{
	...
	auto set_layout_ci = vk::descriptor_set_layout_CI(set_layout_bindings.data(), set_layout_bindings.size());
	vk::check(vkCreateDescriptorSetLayout(instance->vkb.ctx.device, &set_layout_ci, nullptr, &ddgi.write_desc_layout),
			  "Failed to create descriptor set layout");
	set_layout_bindings = {
		vk::descriptor_set_layout_binding(VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER,
										  VK_SHADER_STAGE_RAYGEN_BIT_KHR | VK_SHADER_STAGE_COMPUTE_BIT,
	...
	set_layout_ci = vk::descriptor_set_layout_CI(set_layout_bindings.data(), set_layout_bindings.size());
	vk::check(vkCreateDescriptorSetLayout(instance->vkb.ctx.device, &set_layout_ci, nullptr, &ddgi.read_desc_layout),
			  "Failed to create descriptor set layout");
	...
	for (int i = 0; i < 2; i++) {
		...
		for (int j = 0; j < 2; j++) {
			auto set_allocate_info = vk::descriptor_set_allocate_info(desc_pool, &layouts[j], 1);
			vk::check(vkAllocateDescriptorSets(instance->vkb.ctx.device, &set_allocate_info, sets[j]),
					  "Failed to allocate descriptor sets");
		}
	}
}
// RT
{
	std::vector<VkDescriptorSetLayoutBinding> set_layout_bindings = {
		vk::descriptor_set_layout_binding(VK_DESCRIPTOR_TYPE_STORAGE_IMAGE,
										  VK_SHADER_STAGE_RAYGEN_BIT_KHR | VK_SHADER_STAGE_COMPUTE_BIT, 0),
		vk::descriptor_set_layout_binding(VK_DESCRIPTOR_TYPE_STORAGE_IMAGE,
										  VK_SHADER_STAGE_RAYGEN_BIT_KHR | VK_SHADER_STAGE_COMPUTE_BIT, 1),
	};

	auto set_layout_ci = vk::descriptor_set_layout_CI(set_layout_bindings.data(), set_layout_bindings.size());
	vk::check(vkCreateDescriptorSetLayout(instance->vkb.ctx.device, &set_layout_ci, nullptr, &rt.write_desc_layout),
			  "Failed to create descriptor set layout");
	...
	set_layout_ci = vk::descriptor_set_layout_CI(set_layout_bindings.data(), set_layout_bindings.size());
	vk::check(vkCreateDescriptorSetLayout(instance->vkb.ctx.device, &set_layout_ci, nullptr, &rt.read_desc_layout),
			  "Failed to create descriptor set layout");
	...
	for (int i = 0; i < 2; i++) {
		auto set_allocate_info = vk::descriptor_set_allocate_info(desc_pool, &layouts[i], 1);
		vk::check(vkAllocateDescriptorSets(instance->vkb.ctx.device, &set_allocate_info, sets[i]),
				  "Failed to allocate descriptor sets");
	}
}
...
// DDGI Output
{
	std::vector<VkDescriptorSetLayoutBinding> set_layout_bindings = {vk::descriptor_set_layout_binding(
		VK_DESCRIPTOR_TYPE_STORAGE_IMAGE, VK_SHADER_STAGE_RAYGEN_BIT_KHR | VK_SHADER_STAGE_COMPUTE_BIT, 0)};
	auto set_layout_ci = vk::descriptor_set_layout_CI(set_layout_bindings.data(), set_layout_bindings.size());
	vk::check(vkCreateDescriptorSetLayout(instance->vkb.ctx.device, &set_layout_ci, nullptr, &output.write_desc_layout),
			  "Failed to create descriptor set layout");
	...
	set_layout_ci = vk::descriptor_set_layout_CI(set_layout_bindings.data(), set_layout_bindings.size());
	vk::check(vkCreateDescriptorSetLayout(instance->vkb.ctx.device, &set_layout_ci, nullptr, &output.read_desc_layout),
			  "Failed to create descriptor set layout");
...
// Update
{
	// UBO
	{
		VkWriteDescriptorSet wds = vk::write_descriptor_set(ddgi_uniform.desc_set, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, 0,
															&ddgi_ubo_buffer.descriptor);
		vkUpdateDescriptorSets(instance->vkb.ctx.device, 1, &wds, 0, nullptr);
	}
	// RT Write
	{
		std::vector<VkWriteDescriptorSet> writes = {
			vk::write_descriptor_set(rt.write_desc_set, VK_DESCRIPTOR_TYPE_STORAGE_IMAGE, 0,
									 &rt.radiance_tex.descriptor_image_info),
			vk::write_descriptor_set(rt.write_desc_set, VK_DESCRIPTOR_TYPE_STORAGE_IMAGE, 1,
									 &rt.dir_depth_tex.descriptor_image_info),
		};
		vkUpdateDescriptorSets(instance->vkb.ctx.device, static_cast<uint32_t>(writes.size()), writes.data(), 0,
							   nullptr);
	}
	// RT Read
	{
		...
		std::vector<VkWriteDescriptorSet> writes = {
			vk::write_descriptor_set(rt.read_desc_set, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, 0, &read_infos[0]),
			vk::write_descriptor_set(rt.read_desc_set, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, 1, &read_infos[1]),
		};
		vkUpdateDescriptorSets(instance->vkb.ctx.device, static_cast<uint32_t>(writes.size()), writes.data(), 0,
							   nullptr);
	}
	...
	{
		std::vector<VkWriteDescriptorSet> writes;
		std::vector<VkDescriptorImageInfo> infos = {irr_texes[0].get_descriptor_info(0, VK_IMAGE_LAYOUT_GENERAL),
		...
		for (int i = 0; i < 2; i++) {
			writes.push_back(
				vk::write_descriptor_set(ddgi.write_desc_sets[i], VK_DESCRIPTOR_TYPE_STORAGE_IMAGE, 0, &infos[2 * i]));
			writes.push_back(vk::write_descriptor_set(ddgi.write_desc_sets[i], VK_DESCRIPTOR_TYPE_STORAGE_IMAGE, 1,
													  &infos[2 * i + 1]));
		}
		vkUpdateDescriptorSets(instance->vkb.ctx.device, static_cast<uint32_t>(writes.size()), writes.data(), 0,
							   nullptr);
	...
	// Probe Reads
	{
		...
		std::vector<VkWriteDescriptorSet> writes;
		for (int i = 0; i < 2; i++) {
			writes.push_back(vk::write_descriptor_set(ddgi.read_desc_sets[i], VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER,
													  0, &infos[2 * i]));
			writes.push_back(vk::write_descriptor_set(ddgi.read_desc_sets[i], VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER,
													  1, &infos[2 * i + 1]));
		}
...
VkWriteDescriptorSet write_data_w =
	vk::write_descriptor_set(output.write_desc_set, VK_DESCRIPTOR_TYPE_STORAGE_IMAGE, 0, &wdi);
auto rdi = output.tex.get_descriptor_info(bilinear_sampler, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
VkWriteDescriptorSet write_data_r =
	vk::write_descriptor_set(output.read_desc_set, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, 0, &rdi);
...
void DDGI::create_rt_pipelines() {
	...
	std::vector<Shader> shaders = {{"src/shaders/integrators/ddgi/primary_rays.rgen"},
	...
	settings.desc_layouts = {scene_desc_set_layout, ddgi_uniform.desc_layout, rt.write_desc_layout,
							 rt.read_desc_layout};
...
void DDGI::create_compute_pipelines() {
	...
	std::vector<VkDescriptorSetLayout> layouts = {
		scene_desc_set_layout,	  ddgi.write_desc_layout, ddgi.read_desc_layout,
		ddgi_uniform.desc_layout, rt.read_desc_layout,
	};
	...
	layouts = {ddgi.write_desc_layout, ddgi_uniform.desc_layout};
...
void DDGI::reload() { vis_pipeline->reload(); }
