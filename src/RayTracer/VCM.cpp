...
void VCM::render() {
	...
	// VK_COMMAND_BUFFER_USAGE_ONE_TIME_SUBMIT_BIT); VkClearValue clear_color =
	// { 0.25f, 0.25f, 0.25f, 1.0f }; VkClearValue clear_depth = { 1.0f, 0 };
	// VkViewport viewport = vk::viewport((float)instance->width,
	// (float)instance->height, 0.0f, 1.0f); VkClearValue clear_values[] = {
	// clear_color, clear_depth }; pc_ray.light_pos = scene_ubo.light_pos;
	// pc_ray.light_type = 0;
	...
	// pc_ray.radius = lumen_scene->m_dimensions.radius *
	// lumen_scene->config.radius_factor / 100.f; pc_ray.radius /=
	...
	// pc_ray.grid_res = glm::max(ivec3(diam * float(base_grid_res) / max_comp),
	// ivec3(1));
	//// Prepare
...
void VCM::create_offscreen_resources() {
	...
	// (uint32_t)instance->height, 1 }; settings.format =
...
void VCM::create_descriptors(){
	...
	// vk::check(vkCreateDescriptorSetLayout(instance->vkb.ctx.device,
	// &set_layout_ci,
