...
void MeshLoader::add_model(Model* model) {
	models.push_back(model);
}

void MeshLoader::create_cube_model(Model::Material& material, GLTFModel& cube_model) {
	GLTFModel::Node node;
...
void MeshLoader::load_gltf() {
	for (auto model : models) {
		LUMEN_ASSERT(load_gltf_helper(reinterpret_cast<GLTFModel&>(*model)), "Error loading scene");
...
bool MeshLoader::load_gltf_helper(GLTFModel& model) {
	tinygltf::Model input;
	tinygltf::TinyGLTF loader;
	std::string err;
	std::string warn;
	bool res = loader.LoadASCIIFromFile(&input, &err, &warn, model.path);
	if (!warn.empty()) {
