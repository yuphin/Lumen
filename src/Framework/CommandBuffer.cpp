...
CommandBuffer::CommandBuffer(VulkanContext* ctx, bool begin, 
	...
	this->ctx = ctx;
	this->type = type;
	auto cmd_buf_allocate_info = vk::command_buffer_allocate_info(ctx->cmd_pools[(int)type], level, 1);
...
void CommandBuffer::submit(bool wait_fences) {
	...
	if (wait_fences) {
		...
		vk::check(vkQueueSubmit(ctx->queues[(int)type], 1, &submit_info, VK_NULL_HANDLE),
				  "Queue submission error");
	}
