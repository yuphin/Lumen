...
void SBTWrapper::setup(
	VulkanContext* ctx, uint32_t family_idx,
...
void SBTWrapper::add_indices(
	VkRayTracingPipelineCreateInfoKHR pipeline_infos,
	...
	for (size_t i = 0; i < create_infos.size() + 1; i++) {
		...
		for (uint32_t g = 0; g < info.groupCount; g++) {
			if (info.pGroups[g].type ==
				...
				if (info.pStages[stage_idx].stage ==
					VK_SHADER_STAGE_RAYGEN_BIT_KHR) {
					m_index[eRaygen].push_back(g + group_offset);
					stage_idx++;
				} else if (info.pStages[stage_idx].stage ==
...
void SBTWrapper::create(
	...
	if (pipeline_info.sType ==
		...
		for (auto& i : m_index) {
			if (!i.empty())
				total_group_cnt =
					std::max(total_group_cnt,
							 *std::max_element(std::begin(i), std::end(i)));
		}
		total_group_cnt++;
	...
	auto result = vkGetRayTracingShaderGroupHandlesKHR(
		m_ctx->device, rt_pipeline, 0, total_group_cnt, sbtSize,
		...
		for (auto& e : entry) {
			uint32_t dataHandleSize =
				align_up(static_cast<uint32_t>(
							 m_handle_size + e.second.size() * sizeof(uint8_t)),
						 m_handle_alignment);
	...
	stage[eRaygen] =
		std::vector<uint8_t>(m_stride[eRaygen] * index_count(eRaygen));
	...
	stage[eCallable] =
		std::vector<uint8_t>(m_stride[eCallable] * index_count(eCallable));

	auto copy_handles = [&](std::vector<uint8_t>& buffer,
							std::vector<uint32_t>& indices, uint32_t stride,
							auto& data) {
		...
		for (uint32_t index = 0; index < static_cast<uint32_t>(indices.size());
			...
			memcpy(
				pbuffer,
				shader_handle_storage.data() + (indices[index] * m_handle_size),
				m_handle_size);
			// If there is data for this group index, copy it too
			auto it = data.find(index);
			if (it != std::end(data)) {
				pbuffer += m_handle_size;
				memcpy(pbuffer, it->second.data(),
	...
	copy_handles(stage[eRaygen], m_index[eRaygen], m_stride[eRaygen],
				 m_data[eRaygen]);
	...
	copy_handles(stage[eCallable], m_index[eCallable], m_stride[eCallable],
				 m_data[eCallable]);
	...
	for (uint32_t i = 0; i < 4; i++) {
		if (!stage[i].empty()) {
			m_buffer[i].create(m_ctx, usage_flags, mem_flags,
							   VK_SHARING_MODE_EXCLUSIVE, stage[i].size(),
							   stage[i].data(), true);
		}
	}
}
...
const std::array<VkStridedDeviceAddressRegionKHR, 4> SBTWrapper::get_regions() {
	std::array<VkStridedDeviceAddressRegionKHR, 4> regions{
		get_region(eRaygen), get_region(eMiss), get_region(eHit),
