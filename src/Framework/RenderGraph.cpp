...
static VkPipelineStageFlags get_pipeline_stage(PassType pass_type,
	...
	if ((access_flags & VK_ACCESS_TRANSFER_READ_BIT) ||
...
void RenderPass::register_dependencies(Buffer& buffer,
									   VkAccessFlags dst_access_flags) {
	const bool found = rg->buffer_resource_map.find(buffer.handle) !=
					   rg->buffer_resource_map.end();
	if (!found ||
		(dst_access_flags == VK_ACCESS_SHADER_READ_BIT &&
		 (rg->buffer_resource_map[buffer.handle].second == dst_access_flags))) {
		return;
	}
	RenderPass& opposing_pass =
		rg->passes[rg->buffer_resource_map[buffer.handle].first];
	...
	if (opposing_pass.pass_idx < pass_idx) {
		if (wait_signals_buffer.find(buffer.handle) ==
			wait_signals_buffer.end()) {
		...
		if (opposing_pass.set_signals_buffer.find(buffer.handle) ==
			opposing_pass.set_signals_buffer.end()) {
		...
		buffer_barriers.push_back(
			{buffer.handle, src_access_flags, dst_access_flags});
	}
}

void RenderPass::register_dependencies(Texture2D& tex,
	...
	if (tex.layout == VK_IMAGE_LAYOUT_UNDEFINED ||
		...
		VkAccessFlags dst_access_flags =
			vk::access_flags_for_img_layout(dst_layout);
		if (opposing_pass.pass_idx < pass_idx) {
			...
			// Set source pass dependencies (Signalling pass)
			if (opposing_pass.set_signals_img.find(tex.img) ==
				opposing_pass.set_signals_img.end()) {
				opposing_pass.set_signals_img[tex.img] =
					ImageSyncDescriptor{.old_layout = tex.layout,
										.new_layout = dst_layout,
										.opposing_pass_idx = pass_idx,
										.image_aspect = tex.aspect_flags};
			}
...
static void build_shaders(RenderPass* pass,
						  const std::vector<Shader*>& active_shaders) {
	...
	switch (pass->type) {
		case PassType::Graphics: {
			...
			for (auto& shader : active_shaders) {
				if (pass->rg->shader_cache.find(shader->filename) !=
					pass->rg->shader_cache.end()) {
...
RenderPass& RenderGraph::add_rt(const std::string& name,
								const RTPassSettings& settings) {
	Pipeline* pipeline;
	...
	if (pipeline_cache.find(name) != pipeline_cache.end()) {
		...
		pipeline_cache[name] = {
			std::make_unique<Pipeline>(ctx, pass_idx, name)};
		pipeline = pipeline_cache[name].pipeline.get();
	}
	passes.emplace_back(PassType::RT, pipeline, name, this, pass_idx, settings,
						cached);
	return passes.back();
}

RenderPass& RenderGraph::add_gfx(const std::string& name,
	...
	passes.emplace_back(PassType::Graphics, pipeline, name, this, pass_idx,
						settings, cached);
	return passes.back();
}

RenderPass& RenderGraph::add_compute(const std::string& name,
	...
	passes.emplace_back(PassType::Compute, pipeline, name, this, pass_idx,
						settings, cached);
	return passes.back();
}

RenderPass& RenderGraph::get_current_pass() {
	LUMEN_ASSERT(passes.size(),
				 "Make sure there are passes in the render graph");
	return passes.back();
}
		...
		switch (type) {
			case PassType::Graphics: {
				auto func = [](RenderPass* pass) {
					pass->pipeline->create_gfx_pipeline(
						*pass->gfx_settings, pass->descriptor_counts,
			...
			case PassType::RT: {
				auto func = [](RenderPass* pass) {
					...
					if (!pass->tlas_descriptor_pool) {
						auto pool_size = vk::descriptor_pool_size(
							VK_DESCRIPTOR_TYPE_ACCELERATION_STRUCTURE_KHR, 1);
						auto descriptor_pool_ci =
							vk::descriptor_pool_CI(1, &pool_size, 1);

						vk::check(
							vkCreateDescriptorPool(pass->rg->ctx->device,
												   &descriptor_pool_ci, nullptr,
												   &pass->tlas_descriptor_pool),
							"Failed to create descriptor pool");
						VkDescriptorSetAllocateInfo set_allocate_info{
							VK_STRUCTURE_TYPE_DESCRIPTOR_SET_ALLOCATE_INFO};
						set_allocate_info.descriptorPool =
							pass->tlas_descriptor_pool;
						set_allocate_info.descriptorSetCount = 1;
						set_allocate_info.pSetLayouts =
							&pass->pipeline->tlas_layout;
						vkAllocateDescriptorSets(pass->rg->ctx->device,
												 &set_allocate_info,
												 &pass->tlas_descriptor_set);
					}
					auto descriptor_write = vk::write_descriptor_set(
						pass->tlas_descriptor_set,
						VK_DESCRIPTOR_TYPE_ACCELERATION_STRUCTURE_KHR, 0,
						&pass->tlas_info);
					vkUpdateDescriptorSets(pass->rg->ctx->device, 1,
			...
			case PassType::Compute: {
				auto func = [](RenderPass* pass) {
					pass->pipeline->create_compute_pipeline(
						*pass->compute_settings, pass->descriptor_counts);
				};
				rg->pipeline_tasks.push_back({func, pass_idx});
				break;
			}
...
void RenderPass::write_impl(Texture2D& tex) {
	VkImageLayout target_layout =
		get_target_img_layout(tex, VK_ACCESS_SHADER_WRITE_BIT);
...
void RenderPass::read_impl(Texture2D& tex) {
	VkImageLayout target_layout =
		get_target_img_layout(tex, VK_ACCESS_SHADER_READ_BIT);
	register_dependencies(tex, target_layout);
	rg->img_resource_map[tex.img] = pass_idx;
}
...
void RenderPass::run(VkCommandBuffer cmd) {
	...
	DebugMarker::begin_region(rg->ctx->device, cmd, name.c_str(),
	...
	for (const auto& [k, v] : wait_signals_buffer) {
		LUMEN_ASSERT(
			rg->passes[v.opposing_pass_idx].set_signals_buffer[k].event,
			"Event can't be null");
		buffer_sync.buffer_bariers[i] = buffer_barrier2(
			k, v.src_access_flags, v.dst_access_flags,
			get_pipeline_stage(rg->passes[v.opposing_pass_idx].type,
							   v.src_access_flags),
			get_pipeline_stage(type, v.dst_access_flags));
		buffer_sync.dependency_infos[i] =
			vk::dependency_info(1, &buffer_sync.buffer_bariers[i]);
		wait_events.push_back(
			rg->passes[v.opposing_pass_idx].set_signals_buffer[k].event);
		i++;
	}
	if (wait_events.size()) {
		vkCmdWaitEvents2(cmd, (uint32_t)wait_events.size(), wait_events.data(),
						 buffer_sync.dependency_infos.data());
		for (int i = 0; i < wait_events.size(); i++) {
			vkCmdResetEvent2(cmd, wait_events[i],
							 buffer_sync.buffer_bariers[i].dstStageMask);
		}
	}
	...
	for (auto& barrier : buffer_barriers) {
		...
		buffer_memory_barriers.push_back(
			buffer_barrier2(barrier.buffer, barrier.src_access_flags,
							barrier.dst_access_flags, curr_stage, dst_stage));
	}
	auto dependency_info = vk::dependency_info(
	...
	for (const auto& [k, v] : wait_signals_img) {
		LUMEN_ASSERT(rg->passes[v.opposing_pass_idx].set_signals_img[k].event,
					 "Event can't be null");
		...
		auto src_stage = get_pipeline_stage(
			rg->passes[v.opposing_pass_idx].type, src_access_flags);
		auto dst_stage = get_pipeline_stage(type, dst_access_flags);
		img_sync.img_barriers[i] =
			image_barrier2(k, src_access_flags, dst_access_flags, v.old_layout,
						   v.new_layout, v.image_aspect, src_stage, dst_stage,
						   rg->ctx->indices.gfx_family.value());
		img_sync.dependency_infos[i] =
			vk::dependency_info(1, &img_sync.img_barriers[i]);
		wait_events.push_back(
			rg->passes[v.opposing_pass_idx].set_signals_img[k].event);
		i++;
	}
	if (wait_events.size()) {
		vkCmdWaitEvents2(cmd, (uint32_t)wait_events.size(), wait_events.data(),
						 img_sync.dependency_infos.data());
		for (int i = 0; i < wait_events.size(); i++) {
			vkCmdResetEvent2(cmd, wait_events[i],
							 img_sync.img_barriers[i].dstStageMask);
		}
	}
	...
	if (bound_resources.size()) {
		vkCmdPushDescriptorSetWithTemplateKHR(cmd, pipeline->update_template,
											  pipeline->pipeline_layout, 0,
											  descriptor_infos);
	}
	// Push constants
	if (pipeline->push_constant_size) {
		vkCmdPushConstants(cmd, pipeline->pipeline_layout, pipeline->pc_stages,
						   0, pipeline->push_constant_size, push_constant_data);
	}
	// Run
	if (!disable_execution) {
		switch (type) {
			case PassType::RT: {
				LUMEN_ASSERT(tlas_descriptor_set,
							 "TLAS descriptor set cannot be NULL!");
				...
				if (rt_settings->pass_func) {
					...
					vkCmdTraceRaysKHR(cmd, &regions[0], &regions[1],
									  &regions[2], &regions[3], dims.x, dims.y,
									  dims.z);
				}
				break;
			}
			...
			case PassType::Graphics: {
				...
				VkViewport viewport =
					vk::viewport((float)width, (float)height, 0.0f, 1.0f);
				...
				if (gfx_settings->vertex_buffers.size()) {
					std::vector<VkDeviceSize> offsets(
						gfx_settings->vertex_buffers.size(), 0);
	...
	for (const auto& [k, v] : set_signals_buffer) {
		LUMEN_ASSERT(v.event == nullptr,
					 "VkEvent should be null in the setter");
		VkBufferMemoryBarrier2 mem_barrier = buffer_barrier2(
			k, v.src_access_flags, v.dst_access_flags,
			get_pipeline_stage(type, v.src_access_flags),
			get_pipeline_stage(rg->passes[v.opposing_pass_idx].type,
							   v.dst_access_flags));
		VkDependencyInfo dependency_info = vk::dependency_info(1, &mem_barrier);
		set_signals_buffer[k].event =
			rg->event_pool.get_event(rg->ctx->device, cmd);
	...
	for (const auto& [k, v] : set_signals_img) {
		...
		auto src_access_flags = vk::access_flags_for_img_layout(v.old_layout);
		auto dst_access_flags = vk::access_flags_for_img_layout(v.new_layout);
		auto mem_barrier = image_barrier2(
			k, vk::access_flags_for_img_layout(v.old_layout),
			vk::access_flags_for_img_layout(v.new_layout), v.old_layout,
			v.new_layout, v.image_aspect,
...
void RenderGraph::run(VkCommandBuffer cmd) {
	...
	if (recording) {
		...
		std::set<std::pair<Shader*, RenderPass*>, decltype(cmp)>
			unique_shaders_set;
		...
		for (auto& pass : passes) {
			if (pass.gfx_settings) {
				...
				if (!unique_shaders_set
						 .insert({&pass.compute_settings->shader, &pass})
						 .second) {
					existing_shaders[&pass].push_back(
						&pass.compute_settings->shader);
				}
			}
		}
		...
		for (auto& [pass, shaders] : unique_shaders) {
			futures.push_back(
				ThreadPool::submit(std::bind(&build_shaders, pass, shaders)));
		}
	...
	for (int i = 0; i < passes.size(); i++) {
		buffer_sync_resources[i].buffer_bariers.resize(
			passes[i].wait_signals_buffer.size());
		buffer_sync_resources[i].dependency_infos.resize(
			passes[i].wait_signals_buffer.size());
		img_sync_resources[i].img_barriers.resize(
			passes[i].wait_signals_img.size());
		img_sync_resources[i].dependency_infos.resize(
			passes[i].wait_signals_img.size());
		passes[i].run(cmd);
	}
}
