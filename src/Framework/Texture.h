...
class Texture {
   ...
   protected:
	...
	void cmd_generate_mipmaps(const VkImageCreateInfo& info,
							  VkCommandBuffer cmd);
};
...
class Texture2D : public Texture {
   public:
	...
	//		  uint32_t mip_levels, uint32_t array_layers, VkSampleCountFlagBits,
	//		  VkImageType);
	Texture2D(const std::string& name, VulkanContext* ctx, VkImage image,
