...
void Buffer::create(const char* name, VulkanContext* ctx, VkBufferUsageFlags usage,
	...
	if (use_staging) {
		...
		create(ctx, VK_BUFFER_USAGE_TRANSFER_DST_BIT | usage, mem_property_flags, size, nullptr, false, sharing_mode);
		...
		mem_alloc_info.memoryTypeIndex =
			vk::find_memory_type(&ctx->physical_device, mem_reqs.memoryTypeBits, mem_property_flags);
	...
	if (name) {
		vk::DebugMarker::set_resource_name(ctx->device, (uint64_t)handle, name, VK_OBJECT_TYPE_BUFFER);
		this->name = name;
	}
}
