...
void RTScene::init_resources() {
	...
	loader.init(&vkb.ctx);
	loader.add_model(&cornell_box_model);
	loader.load_gltf();
	...
	resources.cube_pipeline_settings.name = "Cornell Box";
	...
	resources.cube_pipeline_settings.front_face = VK_FRONT_FACE_CLOCKWISE;
	...
	resources.material_render_func = [this](Model::Material* primitive_material,
		...
		vkCmdPushConstants(cmd_buffer, pipeline_layout, VK_SHADER_STAGE_FRAGMENT_BIT,
						   sizeof(SceneResources::ModelPushConst), sizeof(SceneResources::MaterialPushConst), &resources.material_push_const
		);
	};
...
void RTScene::prepare_descriptors() {
	...
	vk::descriptor_pool_size(VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, static_cast<size_t>(loader.num_materials) * 2)
	};
	...
	auto model_push_constant_range = vk::push_constant_range(
		VK_SHADER_STAGE_VERTEX_BIT,
		sizeof(SceneResources::ModelPushConst), 0
	);

	auto material_push_constant_range = vk::push_constant_range(
		VK_SHADER_STAGE_FRAGMENT_BIT,
		sizeof(SceneResources::MaterialPushConst), sizeof(SceneResources::ModelPushConst)
	);

	auto ranges = std::array<VkPushConstantRange, 2>{material_push_constant_range, model_push_constant_range};
	pipeline_layout_ci.pushConstantRangeCount = ranges.size();
...
void RTScene::prepare_descriptor_pool() {
	std::vector<VkDescriptorPoolSize> pool_sizes = {
		vk::descriptor_pool_size(VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER,vkb.ctx.swapchain_images.size()),
...
std::string RTScene::get_asset_path() const {
